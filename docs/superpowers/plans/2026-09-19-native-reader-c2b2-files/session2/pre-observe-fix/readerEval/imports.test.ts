 ...
 * private-window rule, which the harness must CALL rather than copy — a copy could pass here while
 * the real one fails on somebody's screen; `../core/types` is the shared `FrontWindow`;
 * `../main/reader/constants` is the protocol number the helper is checked against.
 */
