...
import {NO_STATS, serialiseError, serialiseResults, INCOMPLETE_OUTCOMES, type EvalMode, type EvalResults} from "./results";
...
describe("what reaches the results file", () => {
  it("carries not one string the helper sent", async () => {
    ...
    const results: EvalResults = {
      schema: 3, mode: "all", nonce: NONCE, repetitions: 2, chromeVariant: "none",
    ...
    // nor the harness's own title grammar, which would mean a title had been copied rather than tested
  ...
  it("does carry the numbers and the codes it exists for", async () => {
    ...
    const written = JSON.parse(serialiseResults({
      schema: 3, mode: "accuracy", nonce: NONCE, repetitions: 2, chromeVariant: "none",
  ...
  it("records the default corner when the option was not given, and the given one when it was", () => {
    const envelope = (env: Record<string, string | undefined>): unknown => {
      const read = readEvalSettings({CLAVE_EVAL_MODE: "accuracy", ...env}, () => "n");
      if (!read.ok) throw new Error(read.code);
      const {position} = read.settings;
      return (JSON.parse(serialiseResults({
        schema: 3, mode: "accuracy", nonce: NONCE, repetitions: 5, chromeVariant: "none",
  ...
  it("records no position at all for a coldstart run", () => {
    const written = JSON.parse(serialiseResults({
      schema: 3, mode: "coldstart", nonce: NONCE, repetitions: 5, chromeVariant: "none",
  ...
  it("carries no string the helper sent in a coldstart run either", async () => {
    ...
    const written = serialiseResults({
      schema: 3, mode: "coldstart", nonce: NONCE, repetitions: 2, chromeVariant: "none",
  ...
  it.each(EVAL_MODES)("records the helper start time in a %s run", (mode: EvalMode) => {
    const written = JSON.parse(serialiseResults({
      schema: 3, mode, nonce: NONCE, repetitions: 1, chromeVariant: "none",
  ...
  it("ignores a field that was never meant to be written, however it got in", () => {
    ...
    const written = serialiseResults({
      schema: 3, mode: "accuracy", nonce: NONCE, repetitions: 1, chromeVariant: "none",
  ...
  it("ignores a field smuggled into a toolbar or an observe ROW", () => {
    const facts = {
      outcome: "ok" as const, host: true, hostBottomPx: 71, private: true, privateBottomPx: 70,
      bandPx: 82, toolbarTextLength: 52, toolbarTextPresent: true, lineCount: 3, stats: NO_STATS
    };
    const written = serialiseResults({
      schema: 3, mode: "all", nonce: NONCE, repetitions: 1, chromeVariant: "none",
      helper: HELPER, permission: "granted", pageServerIpv6: true, position: {x: 40, y: 60},
      accuracy: [],
      toolbar: [{
    ...
    expect(Object.keys(parsed.toolbar?.[0] ?? {}).sort()).toEqual([
      "bandPx", "case", "chromeWaitMs", "displayScale", "expectPrivate", "frontAppSeen", "host",
      "hostBottomPx", "lineCount", "mode", "outcome", "pageRequests", "pageStatus", "private",
      "privateBottomPx", "refusedTitleLength", "scale", "sizeAsStaged", "stageAttempts", "stageMs",
      "staged", "stagedTitleSeen", "stats", "toolbarTextLength", "toolbarTextPresent"
    ]);
    expect(Object.keys(parsed.observe?.[0] ?? {}).sort()).toEqual([
      "app", "bandPx", "case", "chromeWaitMs", "displayScale", "expectPrivate", "frontAppSeen",
      "host", "hostBottomPx", "lineCount", "mode", "outcome", "pageRequests", "pageStatus",
      "private", "privateBottomPx", "refusedTitleLength", "scale", "sizeAsStaged", "stageAttempts",
      "stageMs", "staged", "stagedTitleSeen", "stats", "toolbarTextLength", "toolbarTextPresent"
    ]);
  });
...
describe("the schema version", () => {
  it("is 3, and is written into the file", () => {
    const written = JSON.parse(serialiseResults({
      schema: 3, mode: "coldstart", nonce: NONCE, repetitions: 1, chromeVariant: "none",
      helper: HELPER, permission: "granted", pageServerIpv6: true, position: null,
      accuracy: [], toolbar: [], observe: [],
...
describe("a limited run in the results file", () => {
  const write = (limited: {reason: "LIMITED"; cases: number; of: number} | null): Record<string, unknown> =>
    JSON.parse(serialiseResults({
      schema: 3, mode: "toolbar", nonce: NONCE, repetitions: 1, chromeVariant: "none",
      helper: HELPER, permission: "granted", pageServerIpv6: true, position: {x: 40, y: 60},
      accuracy: [], toolbar: [], observe: [],
  ...
  const withLength = (value: number) => {
    ...
    return JSON.parse(serialiseResults({
      ...
      toolbar: [{
        ...
        outcome: "notStaged", host: null, hostBottomPx: null, private: null, privateBottomPx: null,
