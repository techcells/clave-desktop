 ...
 * and it is DELETED rather than commented, because it was not neutral. Measured: with the lookahead
 * the `{1,2}` run refuses to eat into a longer run of punctuation, so the whole token is left alone
 * and `--.github.com` matches `DOMAIN` whole — an address line. Without it the run is stripped and
 * `.github.com` matches nothing. The reviewer's proof (stripping can never turn a non-match into a
 * match, since every host pattern needs an alphanumeric start or `[`) holds, so removing it can only
 * ever be fail-closed; it was answer-neutral on the 28 rows probed and not on this one.
 */
...
/** Optional `:port`, optional path/query/fragment, then up to three closing marks. */
