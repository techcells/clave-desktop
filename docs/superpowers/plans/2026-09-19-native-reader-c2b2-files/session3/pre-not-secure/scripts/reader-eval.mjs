...
/**
 * The one mode `--reveal-toolbar` is allowed in. Not `all`: that run produces verdicts somebody
 * might act on, and a reveal run is a diagnostic session that can never be accepted.
 */
...
export const USAGE = `usage: pnpm --dir app reader:eval -- <mode> [options]
  ...
  options
    ...
    --reveal-toolbar    PRINT the recognised toolbar strip of each staged window
                        on this terminal. Only in observe mode and only WITHOUT
                        --expect, so a run that reveals can never be accepted.
...
export function parseEvalArgs(argv) {
  ...
  // The reveal fence, the same one the bundle keeps (`REVEAL_NOT_APPLICABLE` in `config.ts`):
  // `observe` only, and only with no expectation, so the run is EXPLORATORY and can never pass.
  if (options.reveal && (options.mode !== REVEALING_MODE || options.expect !== null)) {
...
export async function watchObserve(outDir, nonce, running, io = {}, options = {}) {
   ...
   */
  beat(files.alive);
  for (let tries = 0; tries < URLS_WAIT_TRIES && !running.done; tries += 1) {
    beat(files.alive);
...
async function runEval(options, outDir, nonce) {
  ...
  if (OBSERVING_MODES.includes(options.mode)) {
    await watchObserve(outDir, nonce, running, {}, {reveal: options.reveal === true});
  }
