...
async function main(): Promise<void> {
  ...
  const {mode, nonce, variant, repetitions, seconds, position, limit, expect, host, reveal} = configured.settings;
  /**
   ...
   * exploratory `observe` run, so `reveal` is false in every run that produces a verdict.
   */
  ...
  try {
    ...
    const results: EvalResults = {
      ...
      summary: summarise(mode, runs.accuracy, runs.toolbar, runs.observe, {readyMs, permission}, limited, observeExpect)
    };
