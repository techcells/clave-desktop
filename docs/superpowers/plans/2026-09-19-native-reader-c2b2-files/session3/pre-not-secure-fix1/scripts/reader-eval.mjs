...
export function formatSummary(results) {
  ...
  if (toolbar) {
    ...
    if (isNotSecure(results)) {
      ...
      for (const entry of rows) {
        lines.push(
          `      ${pad(revealCaseName(entry?.case), 48)} outcome ${pad(fixedCode(entry?.outcome), 10)} host ${bool(entry?.host)}` +
  ...
  // And so does an exploratory one. Either marker is enough — the file's top-level boolean or the
  // summary's fixed code — and with either present the word ACCEPTED is never printed, whatever the
  // file's own `accepted` says: the third lock, after `summarise` and `serialiseResults`.
  if (isNotSecure(results)) {
    lines.push(`EXPLORATORY RUN: --not-secure (pages staged under ${NOT_SECURE_HOST}) — can never be accepted`);
  }
  lines.push(results.summary?.accepted === true && !isNotSecure(results) ? "READER_EVAL ACCEPTED" : "READER_EVAL SHORTFALL");
  return lines.join("\n") + "\n";
}

/**
 * Is this the results file of a `toolbar --not-secure` run? Either marker says so: `notSecure: true`
 * at the top, or an `exploratory` object in the summary. Anything present under `exploratory` counts
 * — an unknown reason is a run this terminal must not call accepted either.
 */
export function isNotSecure(results) {
  if (!results || typeof results !== "object") return false;
  if (results.notSecure === true) return true;
  const exploratory = results.summary?.exploratory;
  return exploratory !== null && exploratory !== undefined;
}
