...
use crate::scheduler::{FailReason, LineBox, ReadAnswer, ReadGeometry, ReadStats};
...
pub fn read_line(id: u64, answer: &ReadAnswer, stats: &ReadStats, geometry: Option<&ReadGeometry>) -> String {
...
mod tests {
    ...
    fn a_successful_read_carries_window_text_and_toolbar() {
        ...
        assert_eq!(
            read_line(4, &answer, &NO_STATS, None),
    ...
    fn an_empty_toolbar_strip_is_still_carried() {
        ...
        assert!(read_line(4, &answer, &NO_STATS, None).contains(r#""toolbarText":"""#));
    }
    ...
    fn the_five_failure_reasons() {
        ...
        for (reason, expected) in [
            ...
            assert_eq!(
                read_line(5, &ReadAnswer::Fail(reason), &NO_STATS, None),
    ...
    fn a_successful_read_carries_what_it_cost() {
        let answer = ReadAnswer::Ok { window: window(None), text: "SELECT 1".to_owned(), toolbar: None };
        assert_eq!(
            read_line(4, &answer, &FULL_STATS, None),
    ...
    fn a_black_frame_reports_the_capture_it_did_make() {
        ...
        assert_eq!(
            read_line(5, &ReadAnswer::Fail(FailReason::Black), &stats, None),
    ...
    fn no_other_failure_carries_measurements() {
        // A failure is counted by the app, not measured by it, and `failed` in particular is the
        // answer to "we could not do our job" — there is nothing honest to report about it.
        for reason in [FailReason::Locked, FailReason::Timeout, FailReason::Failed, FailReason::WindowGone] {
            assert_eq!(
                read_line(5, &ReadAnswer::Fail(reason), &FULL_STATS, None),
    ...
    fn a_read_that_measured_nothing_carries_no_stats_object_at_all() {
        ...
        assert!(!read_line(4, &answer, &NO_STATS, None).contains("stats"));
        assert!(!read_line(5, &ReadAnswer::Fail(FailReason::Black), &NO_STATS, None).contains("stats"));
    }
    ...
    fn only_the_fields_that_were_measured_appear() {
        ...
        let answer = ReadAnswer::Ok { window: window(None), text: String::new(), toolbar: None };
        assert!(read_line(4, &answer, &stats, None).contains(r#""stats":{"cacheHit":true}"#));
    }
    ...
    fn nothing_inside_stats_is_ever_a_string() {
        ...
        for line in [read_line(4, &answer, &FULL_STATS, None), read_line(5, &ReadAnswer::Fail(FailReason::Black), &FULL_STATS, None)]
        {
    ...
    fn a_read_that_did_not_ask_for_geometry_carries_none() {
        ...
        let answer = ReadAnswer::Ok { window: window(None), text: "SELECT 1".to_owned(), toolbar: None };
        let line = read_line(4, &answer, &NO_STATS, None);
    ...
    fn a_read_that_asked_carries_a_box_per_line_and_the_band() {
        ...
        assert_eq!(
            read_line(4, &answer, &NO_STATS, Some(&measured())),
            concat!(
                r#"{"id":4,"lines":["#,
    ...
    fn the_band_is_a_number_inside_stats_beside_the_rest() {
        let answer = ReadAnswer::Ok { window: window(None), text: String::new(), toolbar: None };
        let line = read_line(4, &answer, &FULL_STATS, Some(&measured()));
    ...
    fn a_window_with_no_measured_band_carries_its_boxes_and_no_band() {
        ...
        let answer = ReadAnswer::Ok { window: window(None), text: String::new(), toolbar: None };
        let line = read_line(4, &answer, &NO_STATS, Some(&geometry));
    ...
    fn a_cache_hit_carries_no_boxes_at_all() {
        ...
        let stats = ReadStats { cache_hit: Some(true), ..NO_STATS };
        let answer = ReadAnswer::Ok { window: window(None), text: "SELECT 1".to_owned(), toolbar: None };
        let line = read_line(4, &answer, &stats, Some(&geometry));
        assert!(!line.contains("lines"));
    ...
    fn no_failure_ever_carries_boxes_or_a_band() {
        ...
        for reason in [
            ...
            let line = read_line(5, &ReadAnswer::Fail(reason), &FULL_STATS, Some(&measured()));
    ...
    fn text_is_escaped_so_one_answer_is_always_one_line() {
        ...
        let line = read_line(6, &answer, &NO_STATS, None);
