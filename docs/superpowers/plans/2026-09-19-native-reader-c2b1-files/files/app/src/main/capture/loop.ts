...
export interface CaptureLoop {
  ...
  running(): boolean;
  /**
   * How many cycles ended in each outcome since the last `start()`. Numbers only. Per run and not
   * per object, because the one caller writes them out when the run ends (`CAPTURE_OFF`): a reader
   * of that line takes it as "what that run did", and lifetime totals would make every line after
   * the first a sum of runs that had already been written out.
   */
...
export function createCaptureLoop(deps: {
  ...
  let failures: number[] = [];
  let stats: Partial<Record<CycleOutcome, number>> = {};
  ...
  async function cycle(mine: number): Promise<CycleOutcome> {
    try {
      ...
      if (idle >= AWAY_AFTER_SECONDS) return "userAway";
      ...
      if (outcome.reason === "unchanged") return "unchanged";
      ...
      return outcome.reason === "empty" ? "empty" : "notKept";
    } catch (error) {
      ...
      if (mine !== generation) return "stopped";
      noteFailure();
      return error instanceof ReaderCallTimeout ? "timeout" : "failed";
    }
  }
