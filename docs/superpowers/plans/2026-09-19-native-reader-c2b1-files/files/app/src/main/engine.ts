...
import {createCaptureLoop, type BarrenCounts, type CycleOutcome} from "./capture/loop";
...
/**
 * Every outcome the capture loop can report has a count key of its own, so the tallies that reach the
 * log keep their names. A new `CycleOutcome` without a `LOG_COUNT_KEYS` entry stops typecheck here
 * rather than being silently dropped by `log.event`'s closed-set check at runtime.
 */
