...
_Note 2026-09-22 (plan C-2b-2 measurements):_ step 7's band, measured on the Rust-greyscale path for the
first time (record: `docs/superpowers/reviews/2026-09-21-native-reader-c2b2-measurements.md`; results in
`app/reader-eval/out/`, numbers only). NO band-rule change was made: plan decision E3's condition (a
private badge measured below the band, or a lost host) never arose.

**The band as measured.** Chrome 153.0.8010.52: `bandPx` 82 at 1x, 164 at 2x (= 82 pt x 2, carried in
points x display scale as this section says). Safari 27.0: `bandPx` 41 at 1x; Safari at 2x was not run.
Badge positions inside the band: Chrome `privateBottomPx` 70-75 at 1x (`toolbar-c1a1e7e0357d.json`,
`toolbar-49b1f6b62fab.json`), 72 with the bookmarks bar shown (`toolbar-267e2480123c.json`), 73-74 in the
owner's own Chrome with a tab group and the side panel open (`observe-fbebe0578832.json`), 143-148 at 2x
(`toolbar-39912ad56508.json`, `hostBottomPx` 141-152); Safari badge bottom 31-34 px in every observe run
(`observe-c3d2022f1708.json`, `observe-2b2934df8283.json`, `observe-b49ea1a6dedd.json`). Counts, all at
the spec's thresholds: Chrome host 20/20 and private 20/20 and false-private 0/20 in each of the default
(1x and 2x) and bookmarks-bar runs; Safari private 5/5 and false-private 0/5 in each of: compact layout
one tab, compact eight tabs, separate eight tabs. Non-default layouts that all stayed INSIDE the band: the
Chrome bookmarks bar (it sits below the address row, outside the 82 px band, so no band number moves; the
owner confirmed the bar was drawn: "yes"), a tab group, the side panel, pinned extensions (measured
combined in the owner's own Chrome, normal 5/5 read with false-private 0/5, incognito private 5/5;
`observe-732dc6d12b14.json`, `observe-fbebe0578832.json`), and Safari's compact and separate tab layouts
with eight tabs. Not measured: a non-default Chrome theme (the owner uses "Default"; a theme changes
colours, not geometry), an extensions ROW on its own, Safari at 2x.

**O8, owner decision, verbatim "yes" (2026-09-21): a Chrome or Safari read whose toolbar strip shows no
address is not kept (fail closed).** Measured reason: Safari shows the message "Translation Available" in
place of the host for the first seconds after a translatable page loads (seen through the owner-approved
`--reveal-toolbar` exploratory run, `hostDistance` 8-9 on the same three staged page kinds every time), so
for those seconds an excluded SITE could not be recognised and the read would have been kept; the private
badge was unaffected (5/5). Built in `app/src/core/exclusions/` as `showsAddress`, asked PER LINE of the
strip: at most two leading and three trailing letterless, digitless glyph tokens around ONE address token
(domain; `localhost` / `*.localhost`; range-checked IPv4; bracketed IPv6; a single-label host only with a
port or path; port `[1-9][0-9]{0,4}`; host labels never starting or ending with a hyphen); a closed list of
leading browser labels, today exactly "Not secure", English only, stated as a known limit; input capped at
2048 characters; every pattern linear (worst 0.5 ms on 100 KB). Rule order in `after()`: private marker,
then excluded site, then no address (`unknownWindow`); measured browsers only; `extractHosts` and
`siteExcluded` byte-identical to before. Chrome's real omnibox satisfies the rule: `addressLine` 40/40 in
every accepted toolbar run (default 1x, bookmarks bar, 2x). Safari with eight tabs: `addressLine` true in
10 of 20 reads, the misses being translatable pages in their first seconds plus ONE transient miss (host
present, no address line; not reproduced in two reveal runs). Owner-approved copy in KNOWN_LIMITS, his
words "use the shorter one under \"What it cannot do\"": "In Chrome and Safari it only reads a page while
the address is visible." Accepted cost: an occasional lost read; the loop reads again 5 s later.

**Residual hole, documented and pinned:** a tab whose whole title is one address-like token (`Node.js`,
`README.md`, a truncated `README.md ...`, `Not secure Node.js`, `-github.com`) still counts as an address
line. Closing it needs the reader to hand the core the address ROW itself; that is a design item for the
owner, also argued for by the one transient Safari miss above and by one Chrome incognito read in the
"Not secure" run whose address line appears to have merged with the Incognito chip (`addressLine` 39/40,
`toolbar-2022c0133c3a.json`; the read is dropped as private anyway).

**"Not secure" (owner decision "A": the page server stays on loopback; only the eval's throw-away Chrome
profile maps the reserved name `clave-eval.test` to 127.0.0.1; exploratory runs only, never accepted).**
Chrome 153 English at 1x draws the chip at once and it is recognised as ITS OWN line (an icon letter plus
the words "Not Secure"); the address is a separate line: host 40/40, private 20/20, false-private 0/20,
`addressLine` 39/40 (`toolbar-9d4af4de1f44.json` with `--limit 4`, `toolbar-2022c0133c3a.json` full). So
the label-and-address-on-one-line allowance in the closed label list was not needed in those 40 reads and
stays as a harmless allowance. No infobar text inside the band, no HTTPS-first interstitial in incognito.
...
_Note 2026-09-22 (plan C-2b-2 measurements):_ permission in real use, Task 6 of plan C-2b-2, 2026-09-21
23:10 to 2026-09-22 00:06, on the finished install (the Home blocker surface, not onboarding), scripted
model, macOS 27.0; the owner did every System Settings and `tccutil` action himself. Record:
`docs/superpowers/reviews/2026-09-21-native-reader-c2b2-measurements.md`, Session 4.

- **A grant arriving under a running app (D-A after its fix, 10.1 item 3's real-use half).** After
  `tccutil reset ScreenCapture dev.clave.agent.dev` and a fresh launch, Home showed "Screen Recording is
  switched off for this app." with OPEN SYSTEM SETTINGS. Entry switched on; macOS offered Quit & Reopen;
  the owner chose Later: the blocker was gone at once (owner, verbatim: "it was instant"). The app replaced
  its helper by itself (pid 41570 to 41655), no `READER_HELPER_EXIT`, and one minute of reading after it:
  `kept` 20, `failed` 0. After 14 minutes on the blocker (the denied refresh backed off to about one
  helper replacement a minute): button, entry on, Later: "it was instant again". The cure is the client
  replacing the stale helper, as the 2026-09-19 note above says; no app restart was needed either time.
- **Quit & Reopen.** On a re-grant the owner chose it: "it came back by itself"; the same scripted mode
  (`app/.dev-launch.json`), no `NO_READER_YET`; reading, on at the quit, came on again.
- **Revocation (10.1 item 7's real-use half).** Entry switched off while reading: the owner counted
  "7 seconds" until the app showed it. Log order: `CAPTURE_ON`, `PERMISSION_LOST`, `CAPTURE_OFF` (run:
  `kept` 23, `failed` 0, blockers 1); no `READER_PROBLEM`; `READER_HELPER_EXIT` unchanged; the helper was
  replaced quietly. So the revocation goes the way the 2026-09-19 note predicts, through the engine's
  permission poll, not the read path. On re-grant reading RESUMED BY ITSELF (`CAPTURE_ON` within seconds,
  before the owner answered the Quit & Reopen dialog). The `SCStreamErrorDomain` -3801 path (a capture
  refused while the preflight still says yes) was still not exercised.
- **The silent helper replacement.** The denied refresh's replacement of a helper writes NO log code (seen
  by pid changes only: 41570 to 41655, 42204 to 47556 to 47654). Whether that is by design or by omission
  is to be checked against `log.ts`; recorded here as observed.
- **Failed-read detail codes, end to end.** Because the 2026-09-20 `READER_PROBLEM` (5 `failed` in 165
  cycles, cause unknown) could not be explained from the log, every `failed` now carries a closed-set
  detail from the helper to the `CAPTURE_OFF` tallies: `noExpect`, `noGrant`, `captureRefused`,
  `captureTimeout`, `captureError`, `captureNoContent`, `captureNoImage`, `recogniseError`, client-side
  `helperDown`, loop-side `failedFrontWindow`, `failedReadCall`, `timeoutFrontWindow`, `timeoutRead`,
  `failedUnknown`; the failure that trips the five-in-ten-minutes limit is tallied before the problem is
  raised (it was not, a bug found by that run). In the bundle since 2026-09-21 06:3x, reviewed. First
  real-use reading: 2026-09-22 morning, `failed` 1 = `failedCaptureError` 1 in ~900 cycles, during a
  period in which the eval opened and closed 130 windows on the other display; no trip.
...
_Note 2026-09-22 (plan C-2b-2 measurements):_ the lock, watched (Task 6 Step 5, 2026-09-21 23:56 to
2026-09-22 00:05, scripted model). The owner locked the screen for about two minutes: the same helper before
and after (pid 51561), helper CPU time 0:02.43 to 0:02.71, i.e. +0.28 s in 2 min 50 s, about 0.16 % of a
core against about 4 % while reading, so the poll really pauses while locked; `READER_HELPER_EXIT`
unchanged, no `READER_PROBLEM`. Two things differ from what this document and the plan expected: no
`CAPTURE_OFF`/`CAPTURE_ON` pair was logged for the lock (the C-2a run recorded one, see 10.2), and the
run's tallies held NO `locked` key: the locked cycles were counted as `noWindow` (`noWindow` 15, `failed`
0), i.e. `frontWindow` answered null and `read` was never asked, so `read` answering `locked` has still
not been observed. Display sleep (`pmset displaysleepnow`, three times, reading on): `noWindow` 15,
`failed` 0, `black` 0, `locked` 0, same helper, no problem. Machine sleep could not be tested: the owner's
machine has `SleepDisabled 1` (`pmset -g`), a system setting no agent asks him to change for one
measurement; what it would take is `sudo pmset -a disablesleep 0`, the sleep, then `1` again. The
re-entrancy hazard above: no `E_PANIC` was seen. `READER_HELPER_EXIT` read 0 new exits in Session 1
(2026-09-20) and stood at 1 over the log's whole life at Session 3's baseline (2026-09-21 18:30), never
moving in Sessions 3 to 5; when and why that one exit happened is not established in the record (still
open).
...
_Note 2026-09-22 (plan C-2b-2 measurements):_ `reader:eval` has now met a screen. What it measured and how
to re-run it; the record is `docs/superpowers/reviews/2026-09-21-native-reader-c2b2-measurements.md` and
the results files are in `app/reader-eval/out/`, numbers only.

**What it measured.** Accuracy, five repetitions, 18 cases, 90 reads, all `ok` and as staged, ACCEPTED at
1x (`accuracy-ca74052c41fa.json`: chat min 1.0000, ticket 0.9983, code 0.9709, Portuguese 1.0000 with
accents 1.0, terminal 0.9902 for both 140x30 and 72x30) and at 2x (`accuracy-d6fdec16da23.json`: chat
1.0000, ticket 1.0000, code 0.9605, Portuguese 1.0000/1.0, terminal 0.9902). Toolbar, 40 Chrome stagings,
ACCEPTED at 1x default (`toolbar-c1a1e7e0357d.json`, `toolbar-49b1f6b62fab.json`), with the bookmarks bar
(`toolbar-267e2480123c.json`) and at 2x (`toolbar-39912ad56508.json`): host 20/20, private 20/20,
false-private 0/20, `addressLine` 40/40 each. Observe: Safari private 5/5 and normal 0/5 in three layouts,
and the owner's own Chrome with a tab group, the side panel and pinned extensions (files named in the
notes to 5.2). Cold start: `coldstart-67cb5e3fa72f.json`, `readyMs` 150 after a reboot and 13 h 40 min
idle. The same-pixels cache: 90 of 90 immediate re-reads hit, at 1x and at 2x. Timings per read: capture
median 39 ms and recognition median 136 ms at 1x, 27 and 113 ms at 2x.

**How to re-run it.** Modes: `accuracy` (default five repetitions; `--repetitions 1` is a shake-down, never
an acceptance number), `toolbar` (`--variant bookmarks-bar` for the bar; `--not-secure` maps the reserved
name `clave-eval.test` to 127.0.0.1 in the eval's own throw-away Chrome profile only, page server still on
loopback, owner decision "A", such a run can never be accepted), `observe` (now needs `--expect <set>`:
`safari-normal`, `safari-private`, `chrome-normal`, `chrome-private`; it ends as soon as the set is read
and prints progress; `--host` and `hostDistance` for the address host), `coldstart`. `--position <x>,<y>`
places every Chrome staging on another display (integers, -20000..20000; a point off every display's
work area is refused as `POSITION_OFF_DISPLAY`); the 2x runs used `--position 2600,600` with the built-in
Retina display to the right of the external one. `--limit <n>` shortens a run to n stagings; a limited run
can never be accepted. `--reveal-toolbar` prints the recognised toolbar strip of a window the harness
staged itself to the terminal only, in exploratory `observe` or `toolbar --not-secure` runs only, never
written to results, heartbeat-gated and deleted on every exit path; its scope is exactly what the owner
approved ("yes", 2026-09-21: staged windows only, exploratory runs only, terminal only; a second "yes" for
one run on an eight-tab window that also showed his other tabs' titles). A tiling window manager must
not be running: yabai re-tiled every staged window on both days (708x884 pt, then 844x1424 px, for a
1160x640 pt staging), and its runtime title rule still missed 2 of 90 Chrome stagings, so every accepted
run was made with yabai stopped; the harness records `sizeAsStaged` and refuses to accept a run with a
re-tiled staging. The owner's own Chrome windows are minimised or closed during a run. Staged titles are
`CLAVE-EVAL <fixed-width id> <nonce>` (27 characters; the ~73-character titles of the first version did
not survive into the title the window server reports, which made 40 of 40 toolbar stagings `notStaged`).
Results hold numbers, booleans, fixed codes, the harness's own case names and nonce only (schema 6); the
one exception to "no number parsed from a title" is the READY token's two bounded integers
(`readyColumns`, `readyRows`), needed to diagnose Terminal's size.

**Harness repairs the screen demanded** (three loops of plan decision E1, then the observe fixes, each
reviewed with reproducing probes; the guard's rule, the scorer and the thresholds were checked
byte-identical at every review): a READY token in the harness's own title grammar printed after
ENDMARKER (the guard used to pass on the title before the script had printed); `stty size < /dev/tty`
instead of `tput` (which fell back to 80x24 with stdout on a pipe); both terminals staged at 30 rows (the
Retina display cannot hold 40); one automatic re-stage for the first Chrome case of a run; both loopback
families bound as a matched pair; short staged titles; `--limit`; `--expect`; retries on a failed read;
nonce-named run files with a validated 12-hex nonce; `NOT READ` vs `would be KEPT` labels.
...
_Note 2026-09-22 (plan C-2b-2 measurements):_ status after C-2b-2 of rows 2, 8, 9, 14, 15 and 16, the six
rows the measurements moved; every other row above stands as written. Record: `docs/superpowers/reviews/2026-09-21-native-reader-c2b2-measurements.md`;
files in `app/reader-eval/out/`.

| Item | Status after C-2b-2 |
|---|---|
| 2 warm-up before `ready`, the cold cost re-measured | **Verified with number, one caveat.** After a reboot (2026-09-19 23:41) and 13 h 40 min of idle, the first helper start (2026-09-20 13:22, `coldstart-67cb5e3fa72f.json`) had `readyMs` 150 and the next, a minute later, 135; no ~45 s cold cost. Whether some other process used Vision text recognition in between is unknown. Every later helper start in the plan was warm (e.g. 165 ms on 2026-09-21). What triggers the 43-45 s cost is therefore still open, but a reboot plus a long idle does not |
| 8 per-browser band in points x scale | **Verified with number.** Chrome `bandPx` 82 at 1x and 164 at 2x (`toolbar-c1a1e7e0357d.json`, `toolbar-39912ad56508.json`); Safari 41 at 1x (`observe-c3d2022f1708.json` and the other Safari observe files). Badge inside the band in every run: Chrome `privateBottomPx` 70-75 (1x), 72 (bookmarks bar), 73-74 (owner's Chrome, tab group + side panel), 143-148 (2x); Safari 31-34. No band-rule change. Still open: Safari at 2x; a non-default Chrome theme; an unlisted browser in real use (unit tests only, no other browser installed, Task 9) |
| 9 private word case-insensitively inside the band | **Verified with number.** Chrome private 20/20 and false-private 0/20 in each of four accepted toolbar runs (`toolbar-c1a1e7e0357d.json`, `toolbar-49b1f6b62fab.json`, `toolbar-267e2480123c.json`, `toolbar-39912ad56508.json`) and 5/5, 0/5 in the owner's own Chrome (`observe-fbebe0578832.json`, `observe-732dc6d12b14.json`); Safari private 5/5 and false-private 0/5 in each of compact one tab (`observe-c3d2022f1708.json`, `observe-2258e25f2302.json`), compact eight tabs (`observe-2b2934df8283.json`, `observe-cd70f8c6a0b6.json`) and separate eight tabs (`observe-b49ea1a6dedd.json`, `observe-200f0ae53c41.json`). In the app: a private Safari window in front for a run gave `kept` 0, `notKept` 42; a normal one `kept` 41 |
| 14 budget ~60 MB RSS | **Numbers, still open.** Not one growth curve yet: 22 MB after ~11 min (2026-09-20); 92 to 82 MB over 13.2 min and 93 MB at the end of the day (2026-09-21); the Task 7 samples of 2026-09-22: 66,720 KB to 43,792 KB in one helper over ten minutes, then a new helper 33,424 KB to 101,280 KB over its first ten minutes. The ~60 MB budget is exceeded in real use; whether the planned restart bounds it is what Task 7's day is for (sampling running; 3-4 h by the owner's time) |
| 15 `reader:eval` with repeated reads at fixed sizes | **Verified with number.** Five repetitions, fixed recorded sizes (Chrome 1160x640 pt; Terminal 140x30 and 72x30 rows, 30 rather than 40 because the Retina display cannot hold 40), a fresh staging per repetition. The terminal case, the one phase 0 could not explain (0.8686 once): 0.9902 in all five repetitions at 1x, both widths; 0.9922 / 0.9902 at 2x. Minimum and median per group in `accuracy-ca74052c41fa.json` and `accuracy-d6fdec16da23.json` |
| 16 window-id selection and greyscale in Rust: accuracy, speed, RSS | **Verified with number.** Accuracy on the production combination: chat 1.0000, ticket 0.9983, code 0.9709, Portuguese 1.0000 with accents 1.0, terminal 0.9902 (minimums, 1x); 1.0000, 1.0000, 0.9605, 1.0000/1.0, 0.9902 at 2x. Speed: capture median 39 ms (max 101) and recognition median 136 ms (max 177) at 1x; 27 (max 161) and 113 (max 191) at 2x, so about 175 ms per read, about 3.5 % of one core at the 5 s poll; whole-process helper CPU 4.2 % of one core over 13.2 min of ordinary reading. RSS: row 14 |
...
_Note 2026-09-22 (plan C-2b-2 measurements):_ the list above, line by line, after the sessions of
2026-09-20 to 2026-09-22 (record: `docs/superpowers/reviews/2026-09-21-native-reader-c2b2-measurements.md`;
results in `app/reader-eval/out/`). "Verified with number" names the file; "not testable here" says what
it would take; "still open" is left as it is.

- Retina/2x displays: **verified with number.** `accuracy-d6fdec16da23.json` (chat 1.0000, ticket 1.0000,
  code 0.9605, pt 1.0000/accents 1.0, terminal 0.9902; captures 2320x1280 px for 1160x640 pt) and
  `toolbar-39912ad56508.json` (host 20/20, private 20/20, false-private 0/20, `bandPx` 164), both ACCEPTED
  at the same thresholds, on the built-in display in a 1440x900-point scaled mode. Still open: Safari at 2x.
- A real Developer ID: **still open**, carried to packaging (`docs/HANDOFF.md`).
- What triggers the ~45 s cold cost: **narrowed, still open.** A reboot plus 13 h 40 min idle did NOT
  trigger it: `readyMs` 150 then 135 (`coldstart-67cb5e3fa72f.json`); the caveat is that another process
  may have used Vision recognition in between. So a reboot and a long idle are off the list; the trigger
  itself is not established.
- Non-default browser toolbars: **verified with number** for the bookmarks bar
  (`toolbar-267e2480123c.json`: host 20/20, private 20/20, false-private 0/20, badge 72 px of 82; the
  owner saw the bar) and, combined in the owner's own Chrome, a tab group + the side panel + pinned
  extensions (`observe-732dc6d12b14.json` normal 5/5 read, false-private 0/5; `observe-fbebe0578832.json`
  private 5/5, badge 73-74 px of 82). Still open: a non-default theme (the owner uses Default), an
  extensions row on its own.
- Safari with several tabs: **verified with number.** Eight tabs, staged page in tab 5: compact layout
  normal false-private 0/5 (`observe-cd70f8c6a0b6.json`), private 5/5 (`observe-2b2934df8283.json`);
  separate layout private 5/5 (`observe-b49ea1a6dedd.json`), normal 0/5 (`observe-200f0ae53c41.json`);
  badge bottom 31-34 px in the 41 px band. The host: in the compact layout each tab and the address are
  separate recognised lines; the host is absent only while Safari shows "Translation Available" in its
  place (see the O8 note in 5.2), plus one transient miss.
- Arc and other browsers: **not testable here.** No browser other than Chrome and Safari is installed;
  installing one needs the owner's approval (plan Task 9, his decision pending). The core refusing an
  unmeasured browser has still not been seen in real use.
- Messy real screens and the manual checklist: **partly.** Lock and revocation: verified (notes in 5.3 and
  5.5). Overlapping windows, the owner's real VS Code (gutter check), a second display, a full-screen app,
  the helper killed mid-read: still open (Task 8, planned for 2026-09-22). Real screens in ordinary use
  (scripted model, 2026-09-22 morning, ~900 cycles): kept 387, unchanged 230, notKept 71, black 1, failed 1.
- The capture-in-main fallback as a live fallback: **still open**; outside C-2b-2's scope.
- Run-to-run variance: **verified with number.** Five fresh stagings per case: at 1x chat 1.0000/1.0000
  (min/median), ticket 0.9983/1.0000, code 0.9709/0.9751, pt 1.0000/1.0000, terminal 0.9902 in every
  repetition; at 2x code 0.9605/0.9709. The 0.8686 terminal reading did not recur.
- The accuracy of the production combination in Rust: **verified with number**; the two accepted accuracy
  files and the four accepted toolbar files above; 10.1 row 16 for speed.
- Browsers other than Chrome 153 and Safari 27: **still open**, as the Arc line.
- The helper-side focus detection and `locked`: **partly.** The focus poll pausing while locked: verified
  (helper CPU +0.28 s in 2 min 50 s while locked, about 0.16 % of a core, same helper). `read` answering
  `locked`: NOT observed; the locked cycles were tallied as `noWindow` and the `locked` tally stayed 0. The
  C-2a observation of one `CAPTURE_OFF`/`CAPTURE_ON` pair on a lock did not recur: no pair was logged for
  the lock of 2026-09-21 23:5x.
- The same-pixels shortcut: **verified with number.** The immediate second read of an unchanged staging
  was a cache hit in 90 of 90 at 1x (`accuracy-ca74052c41fa.json`) and 90 of 90 at 2x
  (`accuracy-d6fdec16da23.json`). Its hit rate on real, varied windows is not in the tallies.
- Helper memory growth: **numbers, still open** (10.1 row 14; Task 7 sampling running).
- `black` on real windows: **one number, cause still open.** 0 in every staged read of the plan; 1 in ~900
  real-use cycles on 2026-09-22 (the first real-use `black` since the counter exists), against 3 in 36
  diagnostic reads in C-2a. The 2026-09-22 morning run also held the first named failed stage,
  `failedCaptureError` 1, during the eval's 130 window open/close cycles on the other display.
- The twelve browser names against `kCGWindowOwnerName`: **not testable here** (no such browser installed;
  Task 9).
- D-A after its fix, in real use: **verified.** A grant under a running app cleared the blocker at once
  ("it was instant"), also after 14 minutes on it ("it was instant again"); the replaced helper read
  (`kept` 20, `failed` 0). See the note in 5.3.
- A revocation and the -3801 path: **the revocation verified** ("7 seconds"; `PERMISSION_LOST` +
  `CAPTURE_OFF`, no `READER_PROBLEM`, no helper exit; reading resumed by itself on re-grant). The -3801
  `Refused` -> `failed` path: **still open**, never exercised.
- The helper's CPU share per read: **verified with number.** Capture median 39 ms + recognition median
  136 ms at 1x (27 + 113 at 2x), about 175 ms per read, about 3.5 % of one core at the 5 s poll;
  whole-process 4.2 % over 13.2 min of ordinary reading on 2026-09-21.

The C-2b-1 additions:

- `reader:eval` beyond its fakes: **verified**; see the note in section 7 for what the screen decided
  (Terminal's title sequence works with a READY token; Chrome honours `--window-size` and `--window-position`
  once no tiling window manager interferes; the window server reports ~73-character staged titles
  shortened, so titles are 27 characters now; the bundle path `CLAVE_DEV_ENTRY=reader-eval` works end to end
  and the grant reaches the helper it spawns).
- The three comparisons against real titles, `stats` and `lines`: **verified.** 144 productive cycles
  against real titles on 2026-09-20 and hundreds since; `stats` produced `captureMs`, `recogniseMs`,
  `cacheHit` and `bandPx` on every accepted run; `lines` gave the badge and host pixel boxes above.
- The compile-verified-only lines of `macos/focus.rs` and the re-entrancy hazard: **no `E_PANIC` seen.**
  `READER_HELPER_EXIT`: 0 new exits in Session 1 (2026-09-20); 1 over the log's whole life at Session 3's
  baseline (2026-09-21 18:30) and unchanged through Sessions 3 to 5. When and why that one exit happened is
  not established in the record: still open.
- The launcher inside the real bundle under "Quit & Reopen": **verified.** "it came back by itself", same
  scripted mode from `app/.dev-launch.json`, no `NO_READER_YET` (2026-09-21 23:56).
- Both renderer surfaces as rendered pixels: **partly.** The owner has seen the Home blocker surface
  ("Screen Recording is switched off for this app." with OPEN SYSTEM SETTINGS, and its clearing). The
  onboarding permission step rendered in a real bundle, and the nothing-read line (`READER_NOTHING_TO_READ`
  has never fired), are still open.
- The nothing-read notice against a real day: **still open.** 0 notices in the ~900-cycle morning run of
  2026-09-22 (a six-minute excluded-app stint, correctly under the ten-minute clock). The sleep gap (R-1) is
  not testable here: `SleepDisabled 1` on the owner's machine.
- The dev bundle behind: **closed.** `dev:bundle` was run 2026-09-21 06:3x (after the app was quit; it
  refused with `APP_RUNNING` before that, its guard's first real use), protocol 2 in the bundle; the grant
  survived both re-creations of 2026-09-19 (the helper answered `permission: granted` on 2026-09-20) and the
  owner saw the entry in System Settings on 2026-09-21 ("yes, I saw it in system settings", after the
  re-grant of Task 6).
