...
**Update 2026-09-22 (afternoon) — SUB-PROJECT C IS ACCEPTED.** Owner, verbatim: "I accept both C qualifications" (Task 7 sampled a morning, about 1,170 reads, instead of six hours; Task 6 Step 6 machine sleep not testable with `SleepDisabled 1`). The named gaps stay recorded in the measurements record, not closed. Remaining work across the project: D Task 12 (Google sign-in, cancel, rate-limit path) and PKCE; packaging Task 7 (owner: Developer ID under TeamEx), Task 8, Task 10, the download-page link; pushing the clave-back and clave-front commits (owner). The packaging block below remains the current Next for packaging.
...
**Update 2026-09-22 (packaging, evening) — all seven design decisions are made (bundle id `dev.clave.agent`, copyright "Clave", no updater, the mobile app's mark as icon via `app/build/icon.icns`, model licence Apache-2.0), P1/P2/P6 measured under the self-signed packaged build (ledger M1-M7), Task 9's renderer half built (translocation step, About's licences line, the internal-build marks). The single current Next is OWNER-ONLY: Task 7, enrol in the Apple Developer Program under TeamEx, create the Developer ID Application certificate, store the notarisation credential with `xcrun notarytool store-credentials clave-notary`, and tell the packaging session only the two NAMES. Then Task 8 (`package:sign -- --flavour internal --sign "<identity>"` and `package:artefacts -- --flavour internal --notarize clave-notary`, with a version bump, the llama/ drop and the entitlement ladder), then Task 10 (P1/P2/P6 again plus P7). Also pending for About's "Get the latest version" line: the download page URL and an open-a-URL IPC from D's session. The block below is kept as history.**

**Update 2026-09-22 (packaging, later) — the packaged internal build has been LAUNCHED by the owner from /Applications and works end to end: it starts (after two fixes measured on the spot: helper apps need `disable-library-validation` under a self-signed identity because it has no Team ID; the file-protocol fuse must stay at Electron's default for a file:// renderer), renders, downloads and self-tests the real model (20-30 s), raises the Screen Recording dialog naming "Clave Agent Internal.app", picks up the grant without a restart, and read a real screen for two minutes (kept 110, failed 0). Measurements M1-M5 in the packaging ledger. The single current Next: P6 (revocation) with the owner, then decisions 3-7, then the Developer ID (Task 7). Machine: `/Applications/Clave Agent Internal.app` (self-signed, holds a live Screen Recording grant, data folder `~/Library/Application Support/Clave Agent Internal/` with the 2.7 GB model); remove with `rm -rf` of both plus `tccutil reset ScreenCapture dev.clave.agent.internal` when no longer wanted (owner only). The paragraph below is kept as history.**

**Update 2026-09-22 (packaging session, afternoon) — the single current Next for THIS sub-project: the owner installs the internal build from `app/out/internal/artefacts/Clave-Agent-Internal-0.1.0-<build>-arm64.dmg` (drag to Applications, eject, launch from Finder, never a terminal) and reports whether the tray dot and the window appear; the step list is under "Session plan" in the packaging ledger.** State: packaging design approved (decision 1: two flavours, internal = real reader + real model + stub backend, `Clave Agent Internal.app`, id `dev.clave.agent.internal`, own data folder; release refused until D lands); `@electron/packager` 20.3.0 and `@electron/fuses` 2.1.3 installed with the owner's approval; the pipeline build -> stage (offline pnpm install of node-llama-cpp's closure, checked against the lockfile, pruned; licence file with 146 components incl. the packages esbuild inlines) -> bundle (packager, helper at `Contents/MacOS/clave-reader`, five untrue usage strings removed, asar integrity) -> sign (nine fuses, hardened runtime on 25 Mach-Os, helper without entitlements, main at ladder level 0) -> artefacts (DMG + Applications link, ZIP, checks recorded) runs end to end on this machine, self-signed with "Clave Agent Dev" (about 4 minutes, 2.5 of them codesign timestamps). D's session landed the five start-up edits (launchMode, translocation, openLicences), so a packaged internal build can start; packaging then overwrote D's `translocation.ts` with an equivalent file (incident, ruling R20 in the ledger). The self-exclusion now follows the flavour (core/exclusions edit with the owner's permission). Six independent reviews, all findings fixed or recorded. Owner-only items pending: decisions 3-7 (bundle-id domain, legal name for the copyright, updates, icon, model licence), the Developer Program enrolment under TeamEx, `xcrun notarytool store-credentials`, installing into Applications, every grant. Machine: `app/out/` holds about 800 MB (staging, bundle, DMG, ZIP) and is safe to delete; nothing new in `~/Applications`; the dev bundle is untouched; the dev dist is the one on disk (`dist/build.json` says dev). The block below is kept as history.
...
| `/Applications/Clave Agent Internal.app` + `~/Library/Application Support/Clave Agent Internal/` (packaging, 2026-09-22) | installed by the owner | The packaged internal build (self-signed "Clave Agent Dev", bundle id `dev.clave.agent.internal`), **holds a live Screen Recording grant** and the 2.7 GB model. Keep while packaging measurements run. To remove completely: delete both and `tccutil reset ScreenCapture dev.clave.agent.internal` (owner only). Quit it from the tray before reinstalling a new build |
| `app/out/` (packaging, 2026-09-22) | repo, ignored | About 800 MB: `internal/staging`, `internal/bundle` (the fused, self-signed .app), `internal/artefacts` (DMG, ZIP, report). Rebuilt by the four `package:*` scripts; safe to delete; nothing is granted to it (the running-app guard probes it before every rebuild) |
