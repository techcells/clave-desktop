...
/**
 * `toolbar --not-secure` (owner decision "A", 2026-09-21): the ONE host name the staged toolbar pages
 * use in that variant, and the ONE Chrome rule that makes it reach the page server.
 *
 * **Why it exists.** Chrome draws its "Not secure" label for an `http:` page only when the ORIGIN is
 * one it does not already trust, and it trusts every host this harness has ever staged: the literal
 * `127.0.0.1` and every `*.localhost` name are "potentially trustworthy" origins by the secure-context
 * rules, so no ordinary toolbar run can ever show the label. The label is drawn in the address strip
 * — the very strip the product's site exclusions and address-line rule read — so how it is recognised
 * is something only a staged run under a NON-loopback name can measure.
 *
 * **Why this is still loopback-only.** The name is in `.test`, which RFC 6761 reserves: it never
 * resolves on the network, so a browser WITHOUT the rule below gets a resolution error and sends
 * nothing anywhere. The rule is handed to exactly one process — the eval's own Chrome, launched with
 * its own throw-away `--user-data-dir` — and maps exactly this name to `127.0.0.1`, where the page
 * server is bound exactly as it always was (`server.ts` knows nothing of this). The owner's own
 * browser never sees the rule and is never pointed at the name.
 *
 * **A constant, never an input.** Nothing a command line or an environment variable says can change
 * this string: `--host` keeps its loopback-only grammar (`parseObserveHost` refuses this very name),
 * and `--not-secure` is a flag with no value. One rule, one name, one address: no comma (which would
 * start a second rule), no wildcard, no `EXCLUDE`.
 */
export const NOT_SECURE_HOST = "clave-eval.test";
export const NOT_SECURE_RESOLVER_FLAG = `--host-resolver-rules=MAP ${NOT_SECURE_HOST} 127.0.0.1`;
 ...
 *
 * `notSecure` adds `NOT_SECURE_RESOLVER_FLAG`, as ONE argument, and only for the literal `true`. Absent
 * or `false` — every ordinary run — the argument list is byte-identical to what it was before the
 * option existed; `notSecure.test.ts` pins both lists whole.
 */
export function chromeStageCommand(options: {
  ...
  return {
    ...
    args: [
      ...
      ...(options.notSecure === true ? [NOT_SECURE_RESOLVER_FLAG] : []),
      ...(incognito ? ["--incognito"] : []),
      "--new-window",
