...
/** The only value `CLAVE_EVAL_NOT_SECURE` may hold. Mirrored by `NOT_SECURE_ON` in the CLI. */
...
/**
 * May this run reveal its toolbar strips (`--reveal-toolbar`)? The whole fence, as one value.
 *
 * Exactly two situations, and both are runs that can NEVER be accepted:
 *
 *  - `observe` with no expectation — the verdict is `EXPLORATORY` (finding O1). Not `all`, which
 *    runs the accuracy and toolbar parts and produces a verdict somebody might act on.
 *  - `toolbar` WITH `--not-secure` (owner decision "A", 2026-09-21) — the summary carries
 *    `exploratory: NOT_SECURE`, which alone fails `accepted` (`summary.ts`), and the windows are
 *    ones this harness staged itself in its own throw-away Chrome profile: one tab, a page it wrote,
 *    at an address it minted. An ORDINARY toolbar run stays refused, limited or not.
 *
 * It is a function of its own so that the truth table is a thing a test walks cell by cell
 * (`notSecure.test.ts`) rather than a condition somebody reads.
 */
export function mayReveal(mode: EvalMode, expect: readonly string[] | null, notSecure: boolean): boolean {
  if (mode === "observe") return expect === null;
  if (mode === "toolbar") return notSecure === true;
  return false;
}
...
export interface EvalSettings {
   ...
   *
   * The one privacy widening in this harness, and it is fenced on every side: only in a run that can
   * never be accepted — an exploratory `observe` run (no `--expect`) or a `toolbar --not-secure` run
   * (`mayReveal` above) — only the toolbar band, and only through a file the terminal prints and
   * deletes. See the reveal section of `observe.ts` for the measurement that made it necessary and
   * for what it can still contain.
   */
  reveal: boolean;
  /**
   * Stage the toolbar pages under the reserved name `NOT_SECURE_HOST` instead of the `*.localhost`
   * hosts, with the one resolver rule on the eval's own Chrome (`--not-secure`, `stage.ts`).
   *
   * A boolean and nothing else: the NAME is a constant in code and is never an input. Only in
   * `toolbar`, never beside `--host`, and a run with it can never be accepted.
   */
  notSecure: boolean;
}
...
export function readEvalSettings(
  ...
  /**
   * `--not-secure`: exactly `1`, or nothing at all — the same closed grammar as the reveal flag, and
   * for the same reason. It is a SWITCH: there is no value through which a host name, a rule or an
   * address could arrive, because the name it switches on is a constant (`NOT_SECURE_HOST`).
   *
   * `toolbar` only — it changes what the toolbar pages are staged under and nothing else, so in any
   * other mode it would be an option the run silently ignored — and never beside `--host`: `--host`
   * is the loopback-only option, and a command line naming both is one whose author believes the
   * host he typed is the one being measured. Refused with this option's OWN code, checked before
   * `HOST_NOT_APPLICABLE` so the owner is told which of the two he has to drop.
   */
  const rawNotSecure = env.CLAVE_EVAL_NOT_SECURE;
  const notSecure = rawNotSecure === undefined ? false : rawNotSecure === NOT_SECURE_ON ? true : null;
  if (notSecure === null) return {ok: false, code: "BAD_NOT_SECURE"};
  if (notSecure && (mode !== "toolbar" || rawHost !== undefined)) return {ok: false, code: "NOT_SECURE_NOT_APPLICABLE"};
  ...
  /**
   * The fence (`mayReveal`). An exploratory `observe` run — not `all`, which runs the accuracy and
   * toolbar parts and produces a verdict somebody might act on, and only with NO expectation — or a
   * `toolbar --not-secure` run, which is exploratory by construction. A reveal run is a diagnostic
   * session, and it must be impossible for one to be the run that accepts the sub-project.
   */
  if (reveal && !mayReveal(mode, expect, notSecure)) return {ok: false, code: "REVEAL_NOT_APPLICABLE"};
  /**
