...
/**
 * Fix round 1 (review of 2026-09-21). Each test below names the review's mutant it exists to fail.
 */
describe("the rule on an incognito launch", () => {
  const url = "http://clave-eval.test:51234/chat.html?theme=light&size=14";

  /** Review mutant S3: the rule rode twice on incognito launches; "once" was pinned for a normal window only. */
  it("is there exactly once, and the whole argv is this and nothing else", () => {
    const args = chromeStageCommand({profileDir: PROFILE, incognito: true, window: BROWSER_WINDOW, url, notSecure: true}).args;
    expect(args).toEqual([
      "-na", "Google Chrome", "--args",
      `--user-data-dir=${PROFILE}`,
    ...
    expect(args.filter((entry) => entry.includes("host-resolver"))).toHaveLength(1);
  });

  it("is absent from an ordinary incognito launch, whose argv is what it always was", () => {
    const args = chromeStageCommand({profileDir: PROFILE, incognito: true, window: BROWSER_WINDOW, url}).args;
    expect(args).toEqual([
      "-na", "Google Chrome", "--args", `--user-data-dir=${PROFILE}`, "--no-first-run", "--no-default-browser-check",
      "--incognito", "--new-window", "--window-size=1160,640", "--window-position=40,60", url
    ]);
  });

  it("rides once on every launch of a whole incognito case, through the run", async () => {
    const incognito = TOOLBAR_CASES.find((entry) => entry.mode === "incognito") as ToolbarCase;
    const {deps, opens} = world();
    await runToolbarCase(incognito, {...deps, notSecure: true});
    expect(opens()).toHaveLength(1);
    expect((opens()[0] as readonly string[]).filter((entry) => entry.includes("host-resolver"))).toEqual([FLAG]);
    expect(opens()[0]).toContain("--incognito");
  });
});

describe("the literal true, on the line that feeds both the URL and the launch", () => {
  /** Review mutant R1: `Boolean(deps.notSecure)` in `runToolbarCase`. */
  it.each(["yes", "1", 1, "true", {}, []])("stages the ordinary URL and argv for notSecure = %j", async (loose) => {
    const ordinary = world();
    await runToolbarCase(first, ordinary.deps);
    const {deps, opens} = world();
    const {rows, take} = (() => {
      const seen: unknown[] = [];
      return {rows: seen, take: (given: readonly ObserveRevealRow[]) => { seen.push(given); }};
    })();
    const result = await runToolbarCase(first, {...deps, notSecure: loose as unknown as boolean, reveal: true, writeReveal: take});
    expect(opens()).toEqual(ordinary.opens());
    const args = opens()[0] as readonly string[];
    expect(new URL(args[args.length - 1] as string).hostname).toBe(first.host);
    expect(JSON.stringify(args)).not.toContain("host-resolver");
    expect(JSON.stringify(args)).not.toContain("clave-eval");
    // and the third reveal lock stays shut for it
    expect(rows).toEqual([]);
    expect(result.outcome).toBe("ok");
  });
});

describe("revealing is never implied by the variant, on the bundle side", () => {
  const NEW = () => NONCE;

  /** Review mutant C2: `notSecure !== false` in `mayReveal`. */
  it.each(["1", 1, "true", undefined, null, {}])("may not reveal in toolbar for notSecure = %j", (loose) => {
    expect(mayReveal("toolbar", null, loose as never)).toBe(false);
  });

  /** Review mutant C7: `settings.reveal = reveal || notSecure`. */
  it("reads toolbar --not-secure without the reveal variable as a run that does not reveal", () => {
    const read = readEvalSettings({CLAVE_EVAL_MODE: "toolbar", CLAVE_EVAL_NOT_SECURE: "1"}, NEW);
    expect(read.ok).toBe(true);
    expect(read.ok && read.settings.reveal).toBe(false);
    expect(read.ok && read.settings.notSecure).toBe(true);
    const short = readEvalSettings({CLAVE_EVAL_MODE: "toolbar", CLAVE_EVAL_NOT_SECURE: "1", CLAVE_EVAL_LIMIT: "4"}, NEW);
    expect(short.ok && short.settings.reveal).toBe(false);
  });

  it("and the variant is never implied by revealing", () => {
    const read = readEvalSettings({CLAVE_EVAL_MODE: "observe", CLAVE_EVAL_REVEAL_TOOLBAR: REVEAL_ON}, NEW);
    expect(read.ok && read.settings.notSecure).toBe(false);
  });

  /** A whole run handed a writer but not the setting writes nothing (N5's run-time half). */
  it("writes nothing over a whole not-secure run that was not asked to reveal", async () => {
    const seen: unknown[] = [];
    const {deps} = world();
    const runs = await runMode("toolbar", {...deps, notSecure: true, reveal: false, writeReveal: (rows) => { seen.push(rows); }, limit: 4});
    expect(runs.toolbar).toHaveLength(4);
    expect(seen).toEqual([]);
  });
});

describe("a non-boolean accepted", () => {
  const SENTINEL = "SENTINEL-WINDOW-TITLE";
  const handed = (accepted: unknown): EvalResults => ({
    schema: 6, notSecure: false, mode: "toolbar", nonce: NONCE, repetitions: 1, chromeVariant: "none",
    helper: {readyMs: 10}, permission: "granted", pageServerIpv6: true, position: {x: 40, y: 60},
    accuracy: [], toolbar: perfectToolbarRun(), observe: [],
    summary: {...summarise("toolbar", [], perfectToolbarRun()), accepted: accepted as boolean}
  });

  /** Review, Minor 6: `accepted` was the one field a string could ride through into the file. */
  it("is never copied into the results file: a strict boolean is written", () => {
    const text = serialiseResults(handed(SENTINEL));
    expect(text).not.toContain(SENTINEL);
    expect((JSON.parse(text) as {summary: {accepted: unknown}}).summary.accepted).toBe(false);
    for (const value of [1, "true", {}, [], null, undefined, 0, ""]) {
      expect((JSON.parse(serialiseResults(handed(value))) as {summary: {accepted: unknown}}).summary.accepted).toBe(false);
    }
  });

  /** Review mutant Q3: a summary built without the field is an ORDINARY run, written `null` and left accepted. */
  it("reads an absent summary code as no code, not as a marker", () => {
    const {exploratory: _absent, ...older} = handed(true).summary;
    const written = JSON.parse(serialiseResults({...handed(true), summary: older as EvalResults["summary"]})) as {summary: {accepted: unknown; exploratory: unknown}};
    expect(written.summary.exploratory).toBeNull();
    expect(written.summary.accepted).toBe(true);
  });

  it("leaves the two booleans exactly as they were for an ordinary run", () => {
    expect((JSON.parse(serialiseResults(handed(true))) as {summary: {accepted: unknown}}).summary.accepted).toBe(true);
    expect((JSON.parse(serialiseResults(handed(false))) as {summary: {accepted: unknown}}).summary.accepted).toBe(false);
  });
});

/**
 * `main.ts` again. A pin on a LINE is satisfied by a shadowed variable (review mutant N1b: a block
 * that re-declares `notSecure` as false around the results, every pinned line intact, tsc green).
 * So the name itself is counted: it is bound once, from the validated settings, and used three
 * times — the run, the results, the verdict. Any shadow has to spell the name a fifth time.
 */
describe("the entry point's wiring cannot be shadowed", () => {
  const source = readFileSync(new URL("./main.ts", import.meta.url), "utf8");
  const NL = String.fromCharCode(10);
  const count = (needle: string): number => source.split(needle).length - 1;

  it("spells notSecure exactly four times: bound once, used three times", () => {
    expect(count("notSecure")).toBe(4);
    expect(count("notSecure} = configured.settings;")).toBe(1);
    expect(count("        notSecure," + NL)).toBe(1);                       // the run
    expect(count("      notSecure," + NL + "      mode,")).toBe(1);         // the results
    expect(count("exploratoryOf(notSecure))")).toBe(1);                     // the verdict
    // never assigned, never given a value of its own, never renamed
    for (const form of ["notSecure =", "notSecure:"]) expect(count(form)).toBe(0);
    expect(count("exploratoryOf(")).toBe(1);
    expect(count("configured.settings")).toBe(1);
    expect(count("const configured")).toBe(1);
  });

  /** Review mutant N5: the run told `reveal || notSecure`. */
  it("hands the run the validated reveal setting and nothing derived from the variant", () => {
    expect(count("        reveal," + NL + "        writeReveal: reveal ? (rows) => { revealGate?.reveal(rows); } : undefined,")).toBe(1);
    expect(count("const revealGate: RevealGate | null = reveal ? revealGateFor(nonce) : null;")).toBe(1);
    expect(count("reveal:")).toBe(0);
    expect(count("reveal ||")).toBe(0);
    expect(count("|| reveal")).toBe(0);
    expect(count("|| notSecure")).toBe(0);
  });
});
