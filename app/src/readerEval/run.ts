...
import {accents, accuracy, between, confusions, hostDistance, markerHits} from "./score";
import {
  NO_STAGED_SIZE,
  NO_STATS,
...
export interface RunDeps {
  ...
  /**
   * `toolbar --not-secure` (owner decision "A", 2026-09-21): stage every TOOLBAR case under the one
   * reserved name `NOT_SECURE_HOST` instead of its own `*.localhost` host, with the one resolver rule
   * on the eval's own Chrome launch, and search the strip for that name. See `stage.ts`.
   *
   * A boolean, and only the literal `true` counts. It touches the toolbar stagings and nothing else:
   * an accuracy staging never passes it on and stays on `127.0.0.1`, and `observe` windows are the
   * owner's own browser, which must never be pointed at that name. `config.ts` refuses the option in
   * every mode but `toolbar`; this is the half that holds whatever the caller is.
   */
  notSecure?: boolean;
  /**
   * Called with the revealed toolbar strips, when — and only when — the run was asked for them
   * (`--reveal-toolbar`, refused outside an exploratory `observe` run and a `toolbar --not-secure`
   * one: `mayReveal` in `config.ts`).
   *
   * It is a SEPARATE callback from `writeProgress`, to a separate file, because it carries the one
   * thing every other output of this harness is built to exclude: recognised text. Keeping the two
   * apart is what makes "the results file and the progress file are free of it" a property of the
   * shape rather than of a careful line. Absent — the ordinary case — and nothing is shaped, nothing
   * is kept and nothing is written: the strip dies with the read that produced it, exactly as it did
   * before this flag existed.
   *
   * It lives on `RunDeps` rather than `ObserveDeps` since 2026-09-21, because the toolbar part may
   * now be asked for it too. The accuracy part never looks at it.
   */
  writeReveal?(rows: readonly ObserveRevealRow[]): void;
  /**
   * The validated `--reveal-toolbar` setting, and the SECOND half of the lock on it.
   *
   * `writeReveal` being present is not enough: this must also be `true`, or nothing is shaped and
   * the callback is never called. Two independent conditions for one widening, because the line in
   * `main.ts` that decides whether the callback exists at all is a line no test can execute (it is
   * inside the Electron entry), and the reviewer's probe showed that mutating it to pass the writer
   * on every observing run left the whole suite green. This is the half a test CAN execute. The
   * toolbar part demands a THIRD: `notSecure === true` (`runToolbarCase`).
   */
  reveal?: boolean;
}
...
export function toolbarFacts(
...
): Pick<ToolbarCaseResult, "host" | "hostDistance" | "hostBottomPx" | "addressLine" | "private" | "privateBottomPx" | "bandPx" | "toolbarTextLength" | "toolbarTextPresent" | "lineCount"> {
  ...
  return {
    ...
    // The same strip, measured rather than tested (finding O5). Computed here, from the string in
    // hand, and the string is gone when this function returns: one bounded integer survives it.
    hostDistance: hostDistance(toolbarText, options.host),
    // The product's own address-line rule (owner decision O8), CALLED and never copied — like
    // `hasPrivateToolbarMarker` beside it and for the same reason: a copy could pass here while the
    // real one drops the read on somebody's screen, which is the one thing this field exists to find
    // out. `null` for a read with no strip: the rule was not asked, because there was nothing to ask
    // it about.
    addressLine: toolbarText === null ? null : showsAddress(toolbarText),
...
const EMPTY_FACTS = {
  host: null, hostDistance: null, hostBottomPx: null, addressLine: null, private: null, privateBottomPx: null,
  bandPx: null, toolbarTextLength: null, toolbarTextPresent: false, lineCount: null
} as const;

/**
 * The host a toolbar case is STAGED under and its strip is SEARCHED for: the case's own
 * `*.localhost` name, or — for the literal `true` and nothing else — the one reserved name of the
 * `--not-secure` variant. One function, so the URL and the search cannot disagree.
 *
 * The case keeps its table NAME either way (`normal-app.clave.localhost-chat-light-14`): the name is
 * what the staged id, the page server's counters and the results row are keyed by, and the results
 * file says `notSecure: true` once, at the top, for a reader who needs to know the host in the name
 * was not the host in the address bar.
 */
export function toolbarHostOf(theCase: ToolbarCase, notSecure: boolean | undefined): string {
  return notSecure === true ? NOT_SECURE_HOST : theCase.host;
}

export async function runToolbarCase(
  theCase: ToolbarCase,
  deps: RunDeps,
  /**
   * The strips revealed so far in this run, keyed by case, so the table handed to `writeReveal` is
   * cumulative — the terminal deletes the file at every poll, and a table of one would lose every
   * case but the last to a slow poll. `runToolbar` makes one for the run; a caller that runs one
   * case gets one of its own. Never touched unless all three locks below are open.
   */
  revealed: Map<string, ObserveRevealRow> = new Map()
): Promise<ToolbarCaseResult> {
  const notSecure = deps.notSecure === true;
  const host = toolbarHostOf(theCase, notSecure);
  const stagedTitle = stagedTitleOf(theCase.name, deps.nonce);
  const window = positioned(theCase.window, deps.position ?? DEFAULT_POSITION);
  const chromeWaitMs = await prepareChrome(deps, window);
  ...
  const open = chromeStageCommand({
    ...
    window,
    // Only ever the literal `true`, and only ever on this harness's own launch: `chromeStageCommand`
    // puts the rule beside this run's own `--user-data-dir` and nowhere else.
    notSecure,
    url: pageUrl({
      host,
      port: deps.port,
  ...
  try {
    ...
    const stats = statsOf(answer.body);
    // The reveal, for a read that SUCCEEDED and behind THREE independent conditions: the validated
    // setting, the variant, and a writer. `config.ts` refuses `--reveal-toolbar` in an ordinary
    // toolbar run, but the line in `main.ts` that hands over the writer is one no test can execute —
    // so the run itself refuses a writer it was not told to use, and refuses it in any toolbar run
    // that is not the exploratory variant. Only the strip, the band's own lines and the band:
    // `answer.body.text` — the page — is not passed, exactly as in `runObserve`.
    if (deps.reveal === true && notSecure && deps.writeReveal !== undefined) {
      revealed.set(theCase.name, revealRowOf({
        case: theCase.name,
        toolbarText: toolbarOf(answer.body),
        lines: linesOf(answer.body),
        bandPx: bandPxOf(answer.body)
      }));
      deps.writeReveal([...revealed.values()]);
    }
    return {
      ...base,
      outcome: "ok",
      ...toolbarFacts(answer.body, {host, app: theCase.app}),
      stats,
...
export async function runToolbar(deps: RunDeps, cases: readonly ToolbarCase[] = TOOLBAR_CASES): Promise<ToolbarCaseResult[]> {
  ...
  /** One reveal table for the whole run; empty and never touched in an ordinary one. */
  const revealed = new Map<string, ObserveRevealRow>();
  ...
  for (const theCase of limitedTo(interleavedToolbarCases(cases), deps.limit ?? null)) {
    results.push(await runToolbarCase(theCase, shared, revealed));
  }
  return results;
}
...
export interface ObserveDeps extends RunDeps {
  seconds?: number;
  /**
   * The case NAMES this run was asked for (`--expect`, resolved through `expectationOf`), or `null`
   * for an exploratory run.
   *
   * Two things turn on it, and only two: the run ENDS as soon as every one of them has been read ok
   * (finding O6 — the owner waited twenty minutes for two minutes of work), and `summariseObserve`
   * judges the run against it. It never narrows what is WATCHED: a window the owner opens for a case
   * outside the expected set is still read and still recorded, because a read of one of this
   * harness's own staged titles is a measurement, and throwing it away would be throwing away the
   * one thing the mode is for.
   */
  expect?: readonly string[] | null;
  /**
   * The host the observe URLs were printed with, which is therefore the host the strip is searched
   * for (`--host`, finding O5). Defaults to the loopback address, which is what every run before
   * 2026-09-21 used.
   */
  observeHost?: string;
  /**
   * Called after every read attempt with the whole progress table, for the bundle to write to disk.
   *
   * It is a callback rather than a path because nothing in this file touches the filesystem — the
   * same arrangement as `Stager`. Absent in a unit test and in any caller that has nothing to write
   * to; the run is otherwise identical, which is what `run.test.ts` asserts.
   */
  writeProgress?(rows: readonly ObserveProgressRow[], done: number): void;
}

/** One row of the progress table, built from the six fields the allow-list admits and no others. */
function progressRowOf(row: ObserveCaseResult): ObserveProgressRow {
  return {
    case: row.case,
    outcome: row.outcome,
    host: row.host,
 ...
 * `summariseObserve`.
 *
 * **A case is closed by a SUCCESSFUL read, and by nothing else** (finding O4). Run #2's first private
 * case came back `windowGone` — the helper's own approved-window comparisons refused between the
 * approval and the capture, so nothing was captured — and the harness then never looked at that
 * window again, although the owner had left it in front for another minute. The row was recorded, the
 * summary called it a private window that would have been KEPT, and neither was true. Now a failed
 * read leaves the case open, the next poll tries it again while the window is still there, and only
 * `OBSERVE_READ_ATTEMPTS_MAX` attempts later does it stop. The last attempt's row is what is
 * reported, with its count.
 *
 * **And the run ends as soon as it has what it was asked for** (finding O6). The owner of run #2 did
 * two minutes of work and then sat through eighteen minutes of a watcher with nothing to watch for,
 * because the only way out was the clock. With `--expect` the loop ends the moment every expected
 * case has been read ok; without it, when every case in the table has.
 */
export async function runObserve(deps: ObserveDeps, cases: readonly ObserveCase[] = OBSERVE_CASES): Promise<ObserveCaseResult[]> {
  ...
  const host = deps.observeHost ?? DEFAULT_OBSERVE_HOST;
  const until = deps.now() + seconds * 1_000;
  /** Cases read OK. The only map that ends the run. */
  const done = new Map<string, ObserveCaseResult>();
  /** The last FAILED read of a case, kept so a case that never succeeded is still reported. */
  const failed = new Map<string, ObserveCaseResult>();
  /**
   * The revealed strips, when the run was asked for them, keyed by case so a retried read replaces
   * the one before it. Empty and never touched in an ordinary run — `deps.writeReveal` is absent,
   * so nothing below shapes anything.
   */
  const revealed = new Map<string, ObserveRevealRow>();
  const attempts = new Map<string, number>();
  /**
   * What ends the loop early: every case of the expected set read ok — or, with no expectation, the
   * whole table. A name in `--expect` that is not in `cases` cannot be satisfied and so cannot end
   * the run early; it is `summariseObserve` that reports it missing.
   */
  const wanted = (deps.expect ?? cases.map((theCase) => theCase.name))
    .filter((name) => cases.some((theCase) => theCase.name === name));
  /**
   * A case this run will not look at again: read ok, or out of attempts.
   *
   * The second half is the review's Minor 3. An exhausted case can never enter `done`, so a loop
   * that waited for `done` alone would burn the rest of `--seconds` on a window it has already
   * stopped reading — the owner sits out ten minutes for a result that is already decided. Ending
   * loses nothing: an exhausted case is skipped for the remainder of the run whatever happens to
   * that window, so no further watching could change the answer. The verdict is `INCOMPLETE` either
   * way, because the case is named in `missingCases` — this only stops the waiting.
   */
  const settled = (name: string): boolean =>
    done.has(name) || (attempts.get(name) ?? 0) >= OBSERVE_READ_ATTEMPTS_MAX;
  const report = (): void => {
    if (deps.writeProgress === undefined) return;
    const rows = cases.flatMap((theCase) => {
      const row = done.get(theCase.name) ?? failed.get(theCase.name);
      return row ? [progressRowOf(row)] : [];
    });
    deps.writeProgress(rows, wanted.filter((name) => done.has(name)).length);
  };
  while (deps.now() < until && !wanted.every(settled)) {
    const window = await frontWindowOf(deps.helper);
    for (const theCase of cases) {
      if (done.has(theCase.name)) continue;
      const tried = attempts.get(theCase.name) ?? 0;
      // A window that has already failed its reads is left alone rather than read for the rest of
      // the run: the guard would approve it at every poll, and every poll would be another capture.
      if (tried >= OBSERVE_READ_ATTEMPTS_MAX) continue;
      ...
      const outcome = failureOutcome(answer);
      const readAttempts = tried + 1;
      attempts.set(theCase.name, readAttempts);
      const base = {
        case: theCase.name,
        app: theCase.app,
      ...
      if (answer.ok && outcome === "ok") {
        done.set(theCase.name, {
          ...base, outcome: "ok",
          ...toolbarFacts(answer.body, {host, app: theCase.app}),
          stats: statsOf(answer.body), ...unstaged
        });
        // A case that has now been read is a case with nothing left to report a failure about.
        failed.delete(theCase.name);
        // Only for a read that SUCCEEDED, and only when the run asked: the strip, the band's own
        // lines, and nothing else from this answer. `answer.body.text` — the page — is not passed.
        // BOTH conditions, and in this order: a caller that handed over a writer without saying
        // the run was asked to reveal gets nothing shaped and nothing written.
        if (deps.reveal === true && deps.writeReveal !== undefined) {
          revealed.set(theCase.name, revealRowOf({
            case: theCase.name,
            toolbarText: toolbarOf(answer.body),
            lines: linesOf(answer.body),
        ...
        failed.set(theCase.name, {
          ...base, outcome, ...EMPTY_FACTS,
          stats: answer.ok ? statsOf(answer.body) : NO_STATS, ...unstaged
        });
      }
      report();
      break;
  ...
  return cases.flatMap((theCase) => {
    const result = done.get(theCase.name) ?? failed.get(theCase.name);
