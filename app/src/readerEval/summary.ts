...
/**
 * What makes a whole run exploratory, as a fixed code, or nothing.
 *
 * One cause today: `--not-secure`. Only the literal `true` names it — anything else is an ordinary
 * run — and `main.ts` calls this with the validated setting, so the marker in the results file and
 * the verdict below come from the same boolean.
 */
export function exploratoryOf(notSecure: boolean): ExploratoryRun | null {
  return notSecure === true ? {reason: "NOT_SECURE"} : null;
}
...
export function summarise(
  ...
  /**
   * What makes the WHOLE run exploratory, or nothing (`exploratoryOf`). A `toolbar --not-secure`
   * run staged its pages under a name no ordinary run uses, through a resolver rule no ordinary
   * Chrome has: whatever it counted, it did not measure the thing the thresholds are pass marks for.
   * Like `limited`, its presence alone fails `accepted`, however perfect the rows are — and the
   * toolbar verdict beside it is left exactly as it would have been, so the counts are still read.
   */
  exploratory: ExploratoryRun | null = null
): EvalSummary {
