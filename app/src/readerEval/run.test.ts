...
import {showsAddress} from "../core/exclusions/sites";
...
import {OBSERVE_READ_ATTEMPTS_MAX} from "./observe";
import type {ObserveProgressRow} from "./results";
...
describe("observe mode", () => {
  ...
  /**
   * Finding O5, on the row. The measured run recognised `127.0.0.1` in 3 of 8 Safari reads and the
   * file could only say `host: false`; the distance says WHICH failure it was.
   */
  it("records how far the strip was from the host, not only whether it was there", async () => {
    const near = world({
      front: () => ({app: "Safari", title: stagedTitleOf(first.name, NONCE)}),
      // the `1`s came back as `l`s: the misread a bare IP invites
      answer: () => ({ok: true, window: {app: "Safari", title: "x"}, text: "body", toolbarText: "l27.0.0.l/chat", stats: {bandPx: 41}})
    });
    const row = (await runObserve({...near.deps, seconds: 4}, [first]))[0];
    expect(row?.host).toBe(false);
    expect(row?.hostDistance).toBe(2);

    const absent = world({
      front: () => ({app: "Safari", title: stagedTitleOf(first.name, NONCE)}),
      answer: () => ({ok: true, window: {app: "Safari", title: "x"}, text: "body", stats: {bandPx: 41}})
    });
    const none = (await runObserve({...absent.deps, seconds: 4}, [first]))[0];
    expect(none?.host).toBeNull();
    expect(none?.hostDistance).toBeNull();
  });

  /** `--host`: the strip is searched for the host the owner's URLs actually carried (finding O5). */
  it("searches the strip for the host the run was given, not always the loopback address", async () => {
    const answer = () => ({
      ok: true, window: {app: "Safari", title: "x"}, text: "body",
      toolbarText: "app.clave.localhost:51234/chat.html", stats: {bandPx: 41}
    });
    const {deps} = world({front: () => ({app: "Safari", title: stagedTitleOf(first.name, NONCE)}), answer});
    const asked = (await runObserve({...deps, seconds: 4, observeHost: "app.clave.localhost"}, [first]))[0];
    expect(asked?.host).toBe(true);
    expect(asked?.hostDistance).toBe(0);

    // and the default is what every run before the option used, which this strip does not carry
    const second = world({front: () => ({app: "Safari", title: stagedTitleOf(first.name, NONCE)}), answer});
    const byDefault = (await runObserve({...second.deps, seconds: 4}, [first]))[0];
    expect(byDefault?.host).toBe(false);
  });
});

/**
 * Finding O4. Run #2's first private case came back `windowGone` — the helper's own approved-window
 * comparisons refused between the approval and the capture, so NOTHING was captured — and the case
 * was then closed for the rest of the run although the owner had left the window in front. The row
 * was recorded, and the summary called it a private window that would have been KEPT.
 */
describe("a read that failed does not close the case", () => {
  // The case of run #2 that this is about: a Safari window the owner opened PRIVATE.
  const first = OBSERVE_CASES.find((entry) => entry.app === "Safari" && entry.expectPrivate) as (typeof OBSERVE_CASES)[number];
  const front = () => ({app: "Safari", title: stagedTitleOf(first.name, NONCE)});
  const good = {
    ok: true, window: {app: "Safari", title: "x"}, text: "body", toolbarText: "127.0.0.1 Private",
    lines: [{text: "127.0.0.1 Private", topPx: 16, bottomPx: 37, leftPx: 10, rightPx: 200}],
    stats: {bandPx: 41}
  };

  it("tries again at the next poll while the window is still in front, and the good read wins", async () => {
    const {deps, link} = world({
      front,
      answer: (read) => (read === 1 ? {ok: false, reason: "windowGone"} : good)
    });
    const rows = await runObserve({...deps, seconds: 20}, [first]);
    expect(readLines(link)).toHaveLength(2);
    expect(rows).toHaveLength(1);
    expect(rows[0]?.outcome).toBe("ok");
    expect(rows[0]?.readAttempts).toBe(2);
    // the failed attempt left nothing behind: this row is the read that succeeded
    expect(rows[0]?.private).toBe(true);
  });

  /** A window that fails three reads two seconds apart was not merely moved. Bounded, and reported. */
  it("gives up after a bounded number of attempts and reports the failure with its count", async () => {
    const {deps, link} = world({front, answer: () => ({ok: false, reason: "windowGone"})});
    const rows = await runObserve({...deps, seconds: 60}, [first]);
    expect(readLines(link)).toHaveLength(OBSERVE_READ_ATTEMPTS_MAX);
    expect(rows).toHaveLength(1);
    expect(rows[0]?.outcome).toBe("windowGone");
    expect(rows[0]?.readAttempts).toBe(OBSERVE_READ_ATTEMPTS_MAX);
    // and nothing was read, so nothing about the private badge was measured
    expect(rows[0]?.private).toBeNull();
    const verdict = summarise("observe", [], [], rows, undefined, null, {sets: ["safari-private"], cases: [first.name]}).observe;
    expect(verdict?.privateMissed).toBe(0);
    expect(verdict?.missingCases).toEqual([first.name]);
    expect(verdict?.passed).toBe(false);
  });
});

/**
 * Finding O6. Run #2 cost the owner twenty minutes for two minutes of work, because the only way out
 * of the watch was the clock.
 */
describe("when an observe run ends", () => {
  const [first, second] = OBSERVE_CASES as unknown as [typeof OBSERVE_CASES[number], typeof OBSERVE_CASES[number]];

  it("ends as soon as every EXPECTED case has been read, however long it was given", async () => {
    let asked = 0;
    const {deps} = world({front: () => { asked += 1; return {app: first.app, title: stagedTitleOf(first.name, NONCE)}; }});
    const rows = await runObserve({...deps, seconds: 600, expect: [first.name]}, [first, second]);
    expect(rows).toHaveLength(1);
    // one look at the front window, one read, and out: it had what it was asked for
    expect(asked).toBe(1);
  });

  /** With nothing expected there is nothing to be finished, so the clock is the only way out. */
  it("watches to the end of its time when it was asked for nothing", async () => {
    let asked = 0;
    const {deps} = world({front: () => { asked += 1; return {app: first.app, title: stagedTitleOf(first.name, NONCE)}; }});
    await runObserve({...deps, seconds: 10}, [first, second]);
    expect(asked).toBeGreaterThan(1);
  });

  /**
   * Review, Minor 3. An exhausted case can never enter `done`, so a loop that waited for `done`
   * alone would burn the rest of `--seconds` on a window it has already stopped reading. Ending
   * loses nothing — the case is skipped for the remainder of the run whatever happens to that
   * window — and the verdict is INCOMPLETE either way, because the case is named in `missingCases`.
   */
  it("ends when every expected case is either read or out of attempts, and says the run is incomplete", async () => {
    const bad = OBSERVE_CASES.find((entry) => entry.app === "Safari" && entry.expectPrivate) as (typeof OBSERVE_CASES)[number];
    let asked = 0;
    const {deps} = world({
      front: () => { asked += 1; return {app: bad.app, title: stagedTitleOf(bad.name, NONCE)}; },
      answer: () => ({ok: false, reason: "windowGone"})
    });
    const rows = await runObserve({...deps, seconds: 6000, expect: [bad.name]}, [bad]);
    // three attempts, and then out — not the thousands of polls 6000 seconds would have allowed
    expect(asked).toBe(OBSERVE_READ_ATTEMPTS_MAX);
    expect(rows[0]?.readAttempts).toBe(OBSERVE_READ_ATTEMPTS_MAX);
    const verdict = summarise("observe", [], [], rows, undefined, null, {sets: ["safari-private"], cases: [bad.name]}).observe;
    expect(verdict?.reason).toBe("INCOMPLETE");
    expect(verdict?.passed).toBe(false);
  });

  /** And one exhausted case does not stop the run waiting for the others it was asked for. */
  it("keeps waiting for the other expected cases when one is out of attempts", async () => {
    const [bad, good] = OBSERVE_CASES as unknown as [typeof OBSERVE_CASES[number], typeof OBSERVE_CASES[number]];
    let asked = 0;
    const {deps} = world({
      // the failing window is in front for the first four looks, then the good one is
      front: () => { asked += 1; return asked <= 4
        ? {app: bad.app, title: stagedTitleOf(bad.name, NONCE)}
        : {app: good.app, title: stagedTitleOf(good.name, NONCE)}; },
      answer: (read) => (read <= OBSERVE_READ_ATTEMPTS_MAX
        ? {ok: false, reason: "windowGone"}
        : {ok: true, window: {app: good.app, title: "x"}, text: "body", toolbarText: "127.0.0.1/x", stats: {bandPx: 41}})
    });
    const rows = await runObserve({...deps, seconds: 600, expect: [bad.name, good.name]}, [bad, good]);
    expect(rows).toHaveLength(2);
    expect(rows.find((row) => row.case === bad.name)?.outcome).toBe("windowGone");
    expect(rows.find((row) => row.case === good.name)?.outcome).toBe("ok");
  });

  /** A name that is not in the table cannot be satisfied, and must not hold the run open for ever. */
  it("does not wait on an expected name the table does not have", async () => {
    let asked = 0;
    const {deps} = world({front: () => { asked += 1; return {app: first.app, title: stagedTitleOf(first.name, NONCE)}; }});
    await runObserve({...deps, seconds: 600, expect: [first.name, "safari-private-nonesuch"]}, [first]);
    expect(asked).toBe(1);
  });
});

/**
 * The reveal callback (`--reveal-toolbar`). The two claims are "nothing at all unless it was asked
 * for" and "the strip, never the page".
 */
describe("what the run reveals", () => {
  const first = OBSERVE_CASES[0] as (typeof OBSERVE_CASES)[number];
  const front = () => ({app: first.app, title: stagedTitleOf(first.name, NONCE)});
  const answer = () => ({
    ok: true, window: {app: first.app, title: "x"}, text: "STARTMARKER PAGE-BODY-SECRET ENDMARKER",
    toolbarText: "127.0.0.1/chat.html",
    lines: [
      {text: "127.0.0.1/chat.html", topPx: 16, bottomPx: 37, leftPx: 10, rightPx: 200},
      {text: "PAGE-BODY-SECRET", topPx: 200, bottomPx: 220, leftPx: 10, rightPx: 400}
    ],
    stats: {bandPx: 41}
  });

  /** The ordinary run: no callback, so nothing is shaped, nothing is kept and nothing is written. */
  it("reveals nothing at all when the run was not asked to", async () => {
    const {deps} = world({front, answer});
    const rows = await runObserve({...deps, seconds: 4}, [first]);
    expect(rows[0]?.outcome).toBe("ok");
    // the row itself carries no text of any kind, revealed or not
    expect(JSON.stringify(rows)).not.toContain("127.0.0.1/chat.html");
    expect(JSON.stringify(rows)).not.toContain("SECRET");
  });

  it("hands over the strip and the band's line, and never the page body", async () => {
    const revealed: unknown[] = [];
    const {deps} = world({front, answer});
    await runObserve({...deps, seconds: 4, reveal: true, writeReveal: (rows) => { revealed.push(...rows.map((row) => ({...row}))); }}, [first]);
    expect(revealed).toHaveLength(1);
    const written = JSON.stringify(revealed);
    expect(written).toContain("127.0.0.1/chat.html");
    // the page, and the line below the band that repeats it, are both absent
    expect(written).not.toContain("SECRET");
    expect(revealed[0]).toMatchObject({case: first.name, bandPx: 41, linesInBand: 1});
  });

  /** A read that FAILED showed nothing, so there is nothing to reveal about it. */
  it("reveals nothing for a read that failed", async () => {
    const revealed: unknown[] = [];
    const {deps} = world({front, answer: () => ({ok: false, reason: "windowGone"})});
    await runObserve({...deps, seconds: 8, reveal: true, writeReveal: (rows) => { revealed.push(...rows); }}, [first]);
    expect(revealed).toEqual([]);
  });

  /**
   * The second half of the lock (review, Important 2). The line in `main.ts` that decides whether
   * the writer exists at all is inside the Electron entry and no test can execute it — the
   * reviewer's probe showed that mutating it to pass the writer on every observing run left the
   * whole suite green. So the run itself refuses a writer it was not also told to use.
   */
  it("never calls a reveal writer it was not told to use, even when one is handed to it", async () => {
    const revealed: unknown[] = [];
    const take = (rows: readonly unknown[]) => { revealed.push(...rows); };
    const {deps} = world({front, answer});
    // the setting absent altogether
    await runObserve({...deps, seconds: 4, writeReveal: take}, [first]);
    expect(revealed).toEqual([]);
    // and the setting explicitly off
    const second = world({front, answer});
    await runObserve({...second.deps, seconds: 4, reveal: false, writeReveal: take}, [first]);
    expect(revealed).toEqual([]);
    // the reads really did happen, or this proves nothing
    const third = world({front, answer});
    await runObserve({...third.deps, seconds: 4, reveal: true, writeReveal: take}, [first]);
    expect(revealed).toHaveLength(1);
  });
});

/** The running commentary the terminal prints (finding O6), as the callback `main.ts` supplies. */
describe("what the run reports while it is still watching", () => {
  const first = OBSERVE_CASES.find((entry) => entry.app === "Safari" && entry.expectPrivate) as (typeof OBSERVE_CASES)[number];
  const front = () => ({app: "Safari", title: stagedTitleOf(first.name, NONCE)});

  it("reports after every read attempt, the failed ones included", async () => {
    const reported: {rows: readonly ObserveProgressRow[]; done: number}[] = [];
    const {deps} = world({
      front,
      answer: (read) => (read === 1
        ? {ok: false, reason: "windowGone"}
        : {ok: true, window: {app: "Safari", title: "x"}, text: "body", toolbarText: "127.0.0.1 Private", stats: {bandPx: 41}})
    });
    await runObserve({
      ...deps, seconds: 20, expect: [first.name],
      writeProgress: (rows, done) => { reported.push({rows: rows.map((row) => ({...row})), done}); }
    }, [first]);
    expect(reported).toHaveLength(2);
    expect(reported[0]?.rows[0]).toMatchObject({case: first.name, outcome: "windowGone", asExpected: false, readAttempts: 1});
    expect(reported[0]?.done).toBe(0);
    expect(reported[1]?.rows[0]).toMatchObject({case: first.name, outcome: "ok", asExpected: true, readAttempts: 2});
    expect(reported[1]?.done).toBe(1);
    // The core's own answer travels to the terminal beside the host, by VALUE and not only by key.
    // This strip is an address followed by a word, which the per-line rule does not accept — and a
    // read that failed was never asked at all.
    expect(reported[0]?.rows[0]?.addressLine).toBeNull();
    expect(reported[1]?.rows[0]?.addressLine).toBe(showsAddress("127.0.0.1 Private"));
    expect(reported[1]?.rows[0]?.addressLine).toBe(false);
  });

  /**
   * The allow-list, at the place it is easiest to lose: this callback is handed a row every two
   * seconds while a program that can see the screen is running.
   */
  it("reports seven fields and no others, and not one string the helper sent", async () => {
    const reported: ObserveProgressRow[] = [];
    const {deps} = world({
      front: () => ({app: "Safari", title: `${stagedTitleOf(first.name, NONCE)} LEAK-TITLE`}),
      answer: () => ({
        ok: true, window: {app: "LEAK-APP", title: "LEAK-TITLE"}, text: "LEAK-BODY",
        toolbarText: "LEAK-STRIP 127.0.0.1", lines: [{text: "LEAK-LINE", topPx: 1, bottomPx: 2, leftPx: 3, rightPx: 4}],
        stats: {bandPx: 41}
      })
    });
    await runObserve({
      ...deps, seconds: 4, expect: [first.name],
      writeProgress: (rows) => { reported.push(...rows.map((row) => ({...row}))); }
    }, [first]);
    expect(reported).toHaveLength(1);
    expect(Object.keys(reported[0] ?? {}).sort()).toEqual(
      ["addressLine", "asExpected", "case", "host", "outcome", "private", "readAttempts"]
    );
    expect(reported[0]?.addressLine).toBe(showsAddress("LEAK-STRIP 127.0.0.1"));
    expect(JSON.stringify(reported)).not.toContain("LEAK");
  });

  /** A run with nowhere to report is the same run: the callback is optional and never load-bearing. */
  it("runs identically when nobody is listening", async () => {
    const {deps, link} = world({front, answer: () => ({ok: true, window: {app: "Safari", title: "x"}, text: "body", stats: {bandPx: 41}})});
    const rows = await runObserve({...deps, seconds: 4, expect: [first.name]}, [first]);
    expect(rows).toHaveLength(1);
    expect(readLines(link)).toHaveLength(1);
  });
});
...
describe("observe mode, in Chrome", () => {
  ...
  it("does not call a normal CHROME window private for the word `private` inside an address", async () => {
    ...
    const verdict = summarise(
      "observe", [], [], rows, undefined, null, {sets: ["chrome-normal"], cases: [chromeNormal.name]}
    ).observe;
  ...
  it("does call a SAFARI window private for that same bare word, which is Safari's badge", async () => {
    ...
    expect(summarise("observe", [], [], rows, undefined, null, {sets: ["safari-private"], cases: [safariPrivate.name]})
      .observe?.privateMissed).toBe(0);
  });
...
/**
 * Owner decision O8: the core keeps a Chrome or Safari read only when some LINE of the toolbar strip
 * is an address line. Whether real strips satisfy that is a screen question — these rows are how a
 * staged run answers it, and the only thing that makes the answer worth anything is that the harness
 * CALLS the core's rule rather than carrying its own idea of one.
 */
describe("whether the strip showed an address line", () => {
  const stripOf = (toolbarText: string | null) => {
    const body: Record<string, unknown> = {ok: true, stats: {bandPx: 82}};
    if (toolbarText !== null) body.toolbarText = toolbarText;
    return toolbarFacts(body, {host: "127.0.0.1", app: "Google Chrome"});
  };

  it("is the core's own answer, on a strip the core accepts and one it does not", () => {
    const address = "127.0.0.1:51234/chat.html";
    expect(stripOf(address).addressLine).toBe(showsAddress(address));
    expect(stripOf(address).addressLine).toBe(true);

    const message = "Translation Available";
    expect(stripOf(message).addressLine).toBe(showsAddress(message));
    expect(stripOf(message).addressLine).toBe(false);
  });

  /**
   * The pair that separates the core's rule from any re-implementation of it. `Node.js` is a tab
   * TITLE that looks like an address token, and the core's per-line rule accepts a line that is one
   * such token — the residual hole its own doc comment names. A harness predicate that "obviously"
   * refused it would disagree with the product on the exact case the product documents, and the
   * measurement would be of the wrong rule.
   */
  it("agrees with the core on the line the core's own doc calls its residual hole", () => {
    for (const strip of ["Node.js", "Node.js docs", "README.md", "localhost:3000", "[::1]", "wiki/"]) {
      expect(stripOf(strip).addressLine, strip).toBe(showsAddress(strip));
    }
  });

  /** A multi-line strip: the rule is asked per LINE, so one address line among tab titles is enough. */
  it("takes a strip of several lines the way the core does", () => {
    const lines = ["Node.js docs", "127.0.0.1:51234/chat.html", "Release notes"].join(String.fromCharCode(10));
    expect(stripOf(lines).addressLine).toBe(showsAddress(lines));
    expect(stripOf(lines).addressLine).toBe(true);
  });

  /** No strip is not "no address": the rule was never asked. */
  it("is null when the read came back with no strip at all", () => {
    expect(stripOf(null).addressLine).toBeNull();
    expect(stripOf(null).toolbarTextPresent).toBe(false);
  });

  it("is recorded on an observe row too, from the same rule", async () => {
    const first = OBSERVE_CASES[0] as (typeof OBSERVE_CASES)[number];
    const {deps} = world({
      front: () => ({app: first.app, title: stagedTitleOf(first.name, NONCE)}),
      answer: () => ({
        ok: true, window: {app: first.app, title: "x"}, text: "body",
        toolbarText: "127.0.0.1:51234/chat.html", stats: {bandPx: 41}
      })
    });
    const rows = await runObserve({...deps, seconds: 4}, [first]);
    expect(rows[0]?.addressLine).toBe(true);
    expect(rows[0]?.addressLine).toBe(showsAddress("127.0.0.1:51234/chat.html"));
  });

  /** And a failed read has no strip to ask about. */
  it("is null on a row whose read failed", async () => {
    const first = OBSERVE_CASES[0] as (typeof OBSERVE_CASES)[number];
    const {deps} = world({
      front: () => ({app: first.app, title: stagedTitleOf(first.name, NONCE)}),
      answer: () => ({ok: false, reason: "windowGone"})
    });
    const rows = await runObserve({...deps, seconds: 8}, [first]);
    expect(rows[0]?.outcome).toBe("windowGone");
    expect(rows[0]?.addressLine).toBeNull();
  });
});
