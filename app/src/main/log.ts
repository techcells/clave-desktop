...
export const LOG_CODES = [
  ...
  "LOG_BAD_CODE", "BACKGROUND_TASK_FAILED", "SETTINGS_FILE_UNREADABLE", "PERMISSION_LOST", "DATA_DELETED", "OUTBOX_DISCARDED", "SETTINGS_RESET_FOR_NEW_OWNER", "UPLOAD_REJECTED",
  // A browser sign-in began / ended (`failures` 1 when it did not sign anyone in). No state, port or attempt id, ever.
  "SIGN_IN_GOOGLE_STARTED", "SIGN_IN_GOOGLE_FINISHED",
 ...
 *
 * The `failed*` and `timeout*` keys are the same tallies split by WHICH stage failed (the loop's
 * `FailureTallyKey`). They exist because `failed` alone could not be acted on: reading switched
 * itself off after five failures in nine minutes and nothing said whether the grant had gone, macOS
 * was refusing to capture, a capture was hanging or the recogniser was erroring. They sit ALONGSIDE
 * `failed` and `timeout`, which still count every one — a `failed` cycle bumps `failed` and exactly
 * one `failed*` key, so the split sums back to the total.
 *
 * They are spelled out here as literals, which is the point and not bookkeeping: the guarantee this
 * set makes is that every key in it is a word a reader can find in this codebase. A key ASSEMBLED at
 * run time from a string that arrived over a pipe would satisfy the runtime check below while
 * breaking exactly that guarantee. So the helper's word is checked against a closed set at the port
 * (`ports/reader.ts`), mapped to one of these literals by a table in the loop, and the compile-time
 * assertion in `main/engine.ts` holds the two lists together.
 */
export const LOG_COUNT_KEYS = [
  ...
  "failedCaptureNoContent", "failedCaptureNoImage", "failedRecognise", "failedHelperDown", "failedUnknown",
