...
export interface DataPaths {
  ...
  /** Linux: the screen-share grant (the ScreenCast portal's restore token), encrypted. */
  screenGrant: string;
}
...
export function dataPaths(dataDir: string): DataPaths {
  ...
  return {
    ...
    screenGrant: at("screen-grant.bin")
  };
}
...
export const deletablePaths = (paths: DataPaths): string[] =>
  [paths.session, paths.pool, paths.outbox, paths.sentLog, paths.settings, paths.taxonomy, paths.log, paths.screenGrant];
