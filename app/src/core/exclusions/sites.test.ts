 ...
 * quietly rewrite it: two CJK characters, three Arabic letters, two Sinhala letters, one Cyrillic
 * letter and one Arabic-Indic digit.
 */
...
const CYRILLIC = String.fromCodePoint(0x0436);
const ARABIC_INDIC_DIGIT = String.fromCodePoint(0x0663);
...
describe("showsAddress: a line that is NOT an address", () => {
  ...
  // The LEAD DIGIT and the RANGE are two different guards and each needs its own rows. A browser
  // never renders a zero-padded port - not `:000080`, and not `:00080` or `:080` either - so the
  // port pattern is `[1-9][0-9]{0,4}` and every padded form is refused, which is the fail-closed
  // reading (re-review 3, F3: before that only the SIXTH digit was refused and `ab:080` was an
  // address line). The lead digit is written twice, once in `TAIL` and once in `TAIL_REQUIRED`, so
  // it is pinned through every host grammar that reaches either - a guard pinned through one
  // grammar only is how each of the last four rounds lost one.
  it.each([
    "localhost:000080", "intranet:000080", "127.0.0.1:000080",
    "localhost:00080", "localhost:080", "localhost:08", "localhost:080/x",
    "example.com:080", "a.localhost:080", "127.0.0.1:080", "[::1]:080",
    "intranet:00080", "intranet:080", "ab:080", "wiki:080/x"
  ])("a padded port is not a port: %s", (strip) => expect(showsAddress(strip)).toBe(false));

  // The other side of the same pattern: one to FIVE digits, the first of them not a zero.
  // Every lead digit from 1 to 9 is a lead digit: the rows start with 1, 6, 8 and 9 in both tails,
  // so the class cannot shrink from either end unnoticed.
  it.each([
    "localhost:1", "localhost:80", "localhost:10000", "localhost:65535",
    "localhost:9", "localhost:9000", "localhost:99", "127.0.0.1:9090",
    // `localhost:N` also parses as a single label, so `TAIL` alone is pinned by hosts that cannot.
    "example.com:9", "example.com:10000", "[::1]:9000", "127.0.0.1:65535",
    "example.com:80", "127.0.0.1:80", "[::1]:80",
    "intranet:1", "intranet:80", "intranet:10000", "intranet:65535", "intranet:9", "intranet:9000"
  ])("an unpadded port of one to five digits is a port: %s", (strip) => expect(showsAddress(strip)).toBe(true));

  it.each(["intranet:65536", "[::1]:65536", "[::1]:0"])(
    "and the range is checked through every grammar: %s", (strip) => expect(showsAddress(strip)).toBe(false));
  ...
  /**
   * Re-review 3, F1. The hyphen rule was pinned in `DOMAIN` and at the START of the single label and
   * nowhere else: `LOCALHOST_HOST` could go back to `[a-z0-9-]+`, and the single label could END
   * with a hyphen, with the whole suite green. One row per POSITION (first label, inner label, the
   * label next to the TLD or to `localhost`, start and end of each) per GRAMMAR.
   */
  it.each([
    "a-b-.localhost:3000", "a-.localhost:3000", "a.b-.localhost", "a.-b.localhost", "a.-b.localhost:3000",
    "a-.b.localhost", "app.-clave.localhost:8765/chat.html?th...",
    "wik-/", "wiki-/x", "wiki-:8080", "my-wiki-:8080", "my-wiki-/",
    "my-app-.vercel.app", "my-app.-vercel.app", "my-app.vercel-.app", "a.b-.com"
  ])("the hyphen rule holds in every host grammar and at every position: %s", (strip) => expect(showsAddress(strip)).toBe(false));

  // And it is about a label's ENDS, never its middle - in every grammar. Without the single-label
  // rows a "no hyphen inside a single label" edit was free (re-review 3, H13), and that one costs
  // real reads: `my-wiki/` and `dev-box:8080` are what an intranet host looks like.
  it.each([
    "my-app.vercel.app", "my-app.vercel.app/dashboard", "my-app.vercel.app:443/x", "MY-APP.Vercel.App",
    "sub.my-site.co.uk/path?x=1", "xn--bcher-kva.example", "a--b.com",
    "a-b-c.localhost:3000", "my-app.localhost",
    "my-wiki/", "dev-box:8080", "intranet-om:8080", "a--b/"
  ])("a hyphen INSIDE a label is an ordinary host character: %s", (strip) => expect(showsAddress(strip)).toBe(true));

  /**
   * Re-review 3, F2. "A token carrying any letter or digit is never a glyph" was pinned for `TAIL`
   * and not for `TAIL_REQUIRED`, which has its OWN copy of the closing-marks class: it could go back
   * to ASCII-only, or drop its letter half, in silence. The last row of the first list is a plain
   * Latin letter glued after a single-label port.
   */
  it.each([
    `intranet:8080${CJK}`, `intranet:8080${CYRILLIC}`, `wiki/x ${CJK}`, `intranet:8080${ARABIC_INDIC_DIGIT}`,
    "wiki:127z", "intranet:8080A", "intranet:80x.", "intranet:8080.A"
  ])("nothing with a letter or digit may be glued after a single-label port: %s", (strip) => expect(showsAddress(strip)).toBe(false));

  it.each([`localhost:3000${CJK}`, `localhost:3000${CYRILLIC}`, `localhost:3000${ARABIC_INDIC_DIGIT}`, "localhost:3000A", "127.0.0.1:80z"])(
    "nor after any other port: %s", (strip) => expect(showsAddress(strip)).toBe(false));

  /**
   * Re-review 3, F4. Every glued and spaced bound was pinned 9-13 characters away from its edge, so
   * the off-by-one was free in BOTH directions. Each bound gets the last value it accepts and the
   * first it refuses, side by side.
   */
  it.each([
    // closing marks glued to the token: three, through `TAIL` and through `TAIL_REQUIRED`
    "github.com...", "localhost:3000...", "127.0.0.1...", "[::1]...", "github.com/acme...",
    "intranet:80...", "intranet:8080.,)",
    // a leading glyph token: two characters
    "@@ github.com", "*@ 127.0.0.1", "@@ @@ github.com",
    // a trailing glyph token: three characters
    "github.com @@@", "example.com ...", "github.com @@@ ... ---",
    // a glued leading run: two characters
    "((github.com", "==127.0.0.1"
  ])("the last value each glyph bound accepts: %s", (strip) => expect(showsAddress(strip)).toBe(true));

  it.each([
    "github.com....", "localhost:3000....", "127.0.0.1....", "[::1]....",
    "intranet:80....", "intranet:8080.,).",
    "@@@ github.com", "*@: 127.0.0.1", "@@ @@@ github.com",
    "github.com @@@@", "example.com ....", "github.com @@@ .... ---",
    "(((github.com", "===127.0.0.1"
  ])("and the first value it refuses: %s", (strip) => expect(showsAddress(strip)).toBe(false));

  // The bracketed literal's 45: the longest real IPv6 literal is exactly 45 characters and is an
  // address; 46 is not. (K1 and K3 of re-review 3: the far rows above pin 60, not 46, and nothing
  // pinned that 45 itself is still allowed.)
  it("the bracketed literal is bounded at exactly 45 characters", () => {
    const longestReal = "ffff:ffff:ffff:ffff:ffff:ffff:255.255.255.255";
    expect(longestReal.length).toBe(45);
    expect(showsAddress(`[${longestReal}]`)).toBe(true);
    expect(showsAddress(`[${longestReal}]:8080/x`)).toBe(true);
    const fortyFive = "1:".repeat(22) + "1";
    const fortySix = "1:".repeat(22) + "11";
    expect([fortyFive.length, fortySix.length]).toEqual([45, 46]);
    expect(showsAddress(`[${fortyFive}]`)).toBe(true);
    expect(showsAddress(`[${fortySix}]`)).toBe(false);
  });

  // Re-review 3, F5. The rows above (`[abc]`, `[2026-09-21]`, `[09:12:33]`) all fail on the COLON
  // COUNT; none failed on the character class, so `[^\]]` in its place was free. These have three
  // colons or a `::` and are refused for their characters alone.
  it.each(["[12:30:45:PM]", "[a:b:c:word]", "[fe80not:1:2:3:4]", "[::1 x]", "[::g]", "[1:2:3:-]"])(
    "a bracketed literal holds hex digits, dots and colons only: %s", (strip) => expect(showsAddress(strip)).toBe(false));
