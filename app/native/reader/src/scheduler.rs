...
/// WHICH step answered [`FailReason::Failed`]. A closed set of fixed identifiers, one per path in
/// [`handle_read`] that can answer `failed`, and never anything measured, named or read off a screen.
///
/// Why it exists: `failed` is the one reason the app counts against the reader, and five of them in
/// ten minutes switch reading off. The first real run did exactly that — five failures in nine
/// minutes, 129 good reads around them, the helper never died, no timeouts — and nothing on either
/// side of the pipe could say whether the grant had gone, the window server was refusing, a capture
/// was hanging or Vision was erroring. One word for seven different faults is not a diagnosis, and
/// the app cannot show the user the one sentence that would help ("macOS is refusing to capture")
/// from a word that means all seven.
///
/// It is a strictly additive protocol-2 key on the answer (see `protocol::read_line`): an older main
/// ignores keys it does not know, so nothing about an existing answer changes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FailDetail {
    /// The app did not say which window it approved, so there is nothing this helper may capture.
    NoExpect,
    /// `CGPreflightScreenCaptureAccess` says this process has no Screen Recording grant.
    NoGrant,
    /// The system refused the capture itself (`SCStreamErrorDomain` -3801) although the preflight
    /// said yes: a grant that is stale for this process rather than absent.
    CaptureRefused,
    /// A ScreenCaptureKit completion handler never answered within its wait.
    CaptureTimeout,
    /// ScreenCaptureKit reported an error that was neither a refusal nor a vanished window.
    CaptureError,
    /// The shareable-content query answered with neither a window list nor an error. Distinct from
    /// [`FailDetail::CaptureNoImage`] because it is an earlier step: no screenshot was requested.
    CaptureNoContent,
    /// A completion of the SCREENSHOT that carried neither an image nor an error, or an image we
    /// cannot read pixels from.
    CaptureNoImage,
    /// Vision answered with an error rather than lines.
    RecogniseError,
}

impl FailDetail {
    /// The wire string. camelCase fixed identifiers, matching `FailureDetail` in
    /// `app/src/main/ports/reader.ts`, which is a closed set on that side too.
    pub fn as_str(self) -> &'static str {
        match self {
...
/// Everything one read reports BESIDE its answer: what it cost, which step failed, and — for the
/// evaluation harness alone — where the lines sat.
///
/// One struct rather than three out-parameters, which is what these were until the third arrived and
/// [`handle_read`] reached nine positional arguments. Nothing about the out-parameter reasoning
/// changed and it is still the reason all three live here rather than inside [`ReadAnswer`]: an
/// answer is what a read MEANS, some forty assertions in this module compare one with `assert_eq!`,
/// and a measurement must never be able to make two reads that mean the same thing unequal. What the
/// struct buys is that the three travel together under names instead of by position, and that a
/// fourth can be added without touching a single call site's shape.
///
/// Every field is filled in as the steps run, so a read that stopped early carries only what it
/// truly established: `stats` is `None`s until a capture succeeds, `detail` is `Some` only on a
/// `failed` answer, and `geometry` stays empty unless a read asked for it AND recognised something
/// of its own.
#[derive(Debug, Default)]
pub struct ReadReport<'a> {
    pub stats: ReadStats,
    /// Which step answered [`FailReason::Failed`]. `None` on every other answer — see [`FailDetail`].
    pub detail: Option<FailDetail>,
    /// `Some` only when the evaluation harness asked this read for line boxes (`lines` on the wire).
    /// A borrow rather than an owned value because the caller owns the buffer and reads it back;
    /// see [`ReadGeometry`] for why it exists and why a cache hit leaves it empty.
    pub geometry: Option<&'a mut ReadGeometry>,
}

impl<'a> ReadReport<'a> {
    /// A report for an ordinary read: no geometry asked for, nothing measured yet.
    pub fn new() -> Self {
        Self::default()
    }

    /// A report for a read that also wants line boxes written into `geometry`.
    pub fn measuring(geometry: &'a mut ReadGeometry) -> Self {
        Self { geometry: Some(geometry), ..Self::default() }
    }
}
...
///
/// `report` is every out-value this read produces beside its answer — what it cost, which step
/// failed, and the line boxes if any were asked for. It is filled in as the steps run, so the caller
/// can report what a read cost even when it ended early, and it never affects what is answered. See
/// [`ReadReport`] for why all three live outside [`ReadAnswer`] rather than inside it, and
/// [`FailDetail`] for why `detail` is set on exactly the paths that answer [`FailReason::Failed`] —
/// which is what lets `protocol::read_line` state, structurally, that no other answer can carry one.
pub fn handle_read<P: Platform>(
    ...
    report: &mut ReadReport<'_>,
) -> Option<ReadAnswer> {
    // Answer `failed`, saying which step it was. Every `failed` in this function goes through here,
    // so a path that forgets to say which step it is is a path that does not compile.
    macro_rules! failed {
        ($why:expr) => {{
    ...
    let captured = match platform.capture(window.window_id) {
        Ok(captured) => {
            ...
            report.stats.capture_ms = Some(started.elapsed().as_millis() as u64);
            report.stats.width = Some(captured.frame.width as u64);
            report.stats.height = Some(captured.frame.height as u64);
            captured
        }
        Err(CaptureError::Refused) => {
            ...
            cache.clear();
            failed!(FailDetail::CaptureRefused);
        }
        ...
        //
        // The four arms answer identically and differ only in what they SAY: a hung completion
        // handler, an error the framework reported, a content query that came back empty, and a
        // screenshot that brought no usable picture are four different machines to look at, and the
        // run that made this necessary had no way to tell them apart. Written out one arm each
        // rather than as a catch-all, so that a variant added to `CaptureError` later cannot inherit
        // a detail that does not describe it — it fails to compile here instead.
        Err(CaptureError::Timeout) => {
            cache.clear();
            failed!(FailDetail::CaptureTimeout);
        }
        Err(CaptureError::NoContent) => {
            cache.clear();
            failed!(FailDetail::CaptureNoContent);
        }
        Err(CaptureError::NoImage) => {
            cache.clear();
            failed!(FailDetail::CaptureNoImage);
        }
        Err(CaptureError::Other) => {
            cache.clear();
            failed!(FailDetail::CaptureError);
        }
    };
    ...
    if let Some((text, toolbar)) = cache.lookup(window.window_id, hash) {
        report.stats.cache_hit = Some(true);
        return Some(ReadAnswer::Ok { window, text, toolbar });
    }
    report.stats.cache_hit = Some(false);

    // 6 — recognition, the expensive step.
    checkpoint!();
    let started = Instant::now();
    let Ok(lines) = platform.recognise(&captured) else {
        failed!(FailDetail::RecogniseError);
    };
    report.stats.recognise_ms = Some(started.elapsed().as_millis() as u64);
    ...
    let kept = report.geometry.is_some().then(|| text::cleaned(&ordered));
    ...
    if let (Some(geometry), Some(kept)) = (report.geometry.as_deref_mut(), kept) {
        geometry.lines = Some(
            kept.iter()
                .map(|line| LineBox {
...
mod tests {
    ...
    fn handle(
        ...
        handle_read(fake, cache, refused, budget, cancel, Some(&as_approved(fake)), &mut ReadReport::new())
    }
    ...
    fn read_expecting(
        ...
        let answer = handle_read(fake, &mut cache, &refused, Budget::unlimited(), &cancel, expect, &mut report);
        ((answer, refused), report.stats)
    }
    ...
    fn read_measuring(fake: &FakePlatform) -> (Option<ReadAnswer>, ReadGeometry) {
        ...
        let mut cache = ReadCache::new();
        let mut geometry = ReadGeometry::default();
        ...
        let answer = handle_read(
            ...
            &cancel,
            Some(&expect),
            &mut ReadReport::measuring(&mut geometry),
        );
        (answer, geometry)
    }

    /// Run one read and hand back the answer WITH the detail out-parameter and the stats.
    ///
    /// Separate from [`read`] on purpose: every existing test compares the answer alone, and the
    /// detail must stay something a test has to ask for rather than something that changes what an
    /// answer is equal to.
    fn read_detail(fake: &FakePlatform) -> (Option<ReadAnswer>, Option<FailDetail>, ReadStats) {
        read_detail_expecting(fake, Some(&as_approved(fake)))
    }

    /// The same, against an arbitrary expectation — including `None`, which is the app failing to
    /// say what it approved.
    fn read_detail_expecting(
        fake: &FakePlatform,
        expect: Option<&Expected>,
    ) -> (Option<ReadAnswer>, Option<FailDetail>, ReadStats) {
        let (refused, cancel) = (AtomicBool::new(false), AtomicBool::new(false));
        let mut cache = ReadCache::new();
        let mut report = ReadReport::new();
        let answer = handle_read(fake, &mut cache, &refused, Budget::unlimited(), &cancel, expect, &mut report);
        (answer, report.detail, report.stats)
    }
    ...
    fn survives_the_cache_expecting(fake: &FakePlatform, expect: Option<&Expected>) -> (Option<ReadAnswer>, bool) {
        ...
        let (refused, cancel) = (AtomicBool::new(false), AtomicBool::new(false));
        let answer = handle_read(fake, &mut cache, &refused, Budget::unlimited(), &cancel, expect, &mut ReadReport::new());
    ...
    fn a_switch_during_recognition_throws_the_text_away_and_remembers_nothing() {
        ...
        let mut cache = ReadCache::new();
        let answer =
            handle_read(&moved, &mut cache, &refused, Budget::unlimited(), &cancel, Some(&approved), &mut ReadReport::new());
        assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::WindowGone)));
        assert_eq!(moved.recognise_calls.get(), 1, "it had already been recognised when the user switched");

        let back = FakePlatform::new();
        let again =
            handle_read(&back, &mut cache, &refused, Budget::unlimited(), &cancel, Some(&approved), &mut ReadReport::new());
    ...
    /// EVERY way a capture can fail forgets the last window. Nothing is being read, so nothing
    /// recognised is left behind — spec 5.2's "which returns clear the cache", and the rule the
    /// Privacy screen's "Nothing older than an hour exists anywhere" rests on, since `cache` holds a
    /// whole window's recognised text.
    ///
    /// A sweep and not one case, because this rule is per ARM and the arms multiplied: splitting
    /// `CaptureError::Other` into five variants split one guarded `cache.clear()` into five, of
    /// which a single-case test would still guard one. Deleting the clear from ANY arm has to fail
    /// here, which is what the per-iteration `Shot::Err(error)` buys.
    ///
    /// "EVERY way" is true of the variants that exist, and the list is written out by hand: a
    /// seventh `CaptureError` would not compile in `handle_read` (that `match` has no wildcard
    /// arm), but it WOULD be silently absent from here. Whoever adds one adds it to this list and
    /// to `each_capture_error_says_which_step_failed`, which is enumerated the same way.
    #[test]
    fn every_way_a_capture_can_fail_forgets_the_last_window() {
        // `Gone` answers `windowGone` rather than `failed` — a state of the screen, not a fault —
        // but it clears the cache like all the rest, so the expected answer travels with the case.
        let cases = [
            (CaptureError::Refused, FailReason::Failed),
            (CaptureError::Gone, FailReason::WindowGone),
            (CaptureError::Timeout, FailReason::Failed),
            (CaptureError::NoContent, FailReason::Failed),
            (CaptureError::NoImage, FailReason::Failed),
            (CaptureError::Other, FailReason::Failed),
        ];
        for (error, reason) in cases {
            let fake = FakePlatform::new().shots([Shot::Err(error)]);
            let (answer, survived) = survives_the_cache(&fake);
            assert_eq!(answer, Some(ReadAnswer::Fail(reason)), "error {error:?}");
            assert!(!survived, "{error:?} left recognised text behind");
        }
    }
    ...
    // -- which step failed ----------------------------------------------------------------------
    //
    // The whole of the diagnosis: `failed` is one word for seven faults, and five of them in ten
    // minutes switch reading off. These pin the word each path adds beside it — one test per path,
    // driven end to end through `handle_read`, so a path that stops setting its detail (or starts
    // setting another one's) fails here rather than at the far end of a log nobody can read.

    #[test]
    fn a_read_that_was_told_no_approved_window_says_which_step_failed() {
        let fake = FakePlatform::new();
        let (answer, detail, _) = read_detail_expecting(&fake, None);
        assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::Failed)));
        assert_eq!(detail, Some(FailDetail::NoExpect));
        assert_eq!(fake.capture_calls.get(), 0);
    }

    #[test]
    fn a_read_without_the_grant_says_which_step_failed() {
        let mut fake = FakePlatform::new();
        fake.preflight = false;
        let (answer, detail, _) = read_detail(&fake);
        assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::Failed)));
        assert_eq!(detail, Some(FailDetail::NoGrant));
    }

    /// The four capture errors, each carrying its own word. `Gone` is deliberately not here: it is
    /// `windowGone`, not a failure at all, and the test below says so.
    #[test]
    fn each_capture_error_says_which_step_failed() {
        let cases = [
            (CaptureError::Refused, FailDetail::CaptureRefused),
            (CaptureError::Timeout, FailDetail::CaptureTimeout),
            (CaptureError::NoContent, FailDetail::CaptureNoContent),
            (CaptureError::NoImage, FailDetail::CaptureNoImage),
            (CaptureError::Other, FailDetail::CaptureError),
        ];
        for (error, expected) in cases {
            let fake = FakePlatform::new().shots([Shot::Err(error)]);
            let (answer, detail, _) = read_detail(&fake);
            assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::Failed)), "error {error:?}");
            assert_eq!(detail, Some(expected), "error {error:?}");
        }
    }

    #[test]
    fn a_recogniser_error_says_which_step_failed() {
        let mut fake = FakePlatform::new();
        fake.recognise_ok = false;
        let (answer, detail, _) = read_detail(&fake);
        assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::Failed)));
        assert_eq!(detail, Some(FailDetail::RecogniseError));
    }

    /// Nothing that is not `failed` leaves a detail behind, whichever path it took. Written as a
    /// sweep of every non-`failed` answer `handle_read` can give rather than one test each, because
    /// the claim is about all of them at once: the detail is a fact about failures, and an answer
    /// that is not one must not carry a word explaining a failure that did not happen.
    #[test]
    fn no_answer_other_than_failed_leaves_a_detail_behind() {
        let mut locked = FakePlatform::new();
        locked.locked = true;
        let cases: [(&str, FakePlatform); 5] = [
            ("ok", FakePlatform::new()),
            ("black", FakePlatform::new().shots([Shot::Pixels(0)])),
            ("locked", locked),
            ("windowGone: nothing in front", FakePlatform::new().window(None)),
            ("windowGone: the window vanished", FakePlatform::new().shots([Shot::Err(CaptureError::Gone)])),
        ];
        for (what, fake) in cases {
            let (answer, detail, _) = read_detail(&fake);
            assert!(!matches!(answer, Some(ReadAnswer::Fail(FailReason::Failed))), "{what}: {answer:?}");
            assert_eq!(detail, None, "{what} left a detail behind");
        }
    }

    /// A timeout and a cancel are the two answers reached through `checkpoint!`, which never touches
    /// the detail. Kept apart from the sweep above because staging them needs a spent budget and a
    /// cancel flag rather than a platform.
    #[test]
    fn a_timed_out_or_cancelled_read_leaves_no_detail_behind() {
        let fake = FakePlatform::new();
        let (refused, cancel) = (AtomicBool::new(false), AtomicBool::new(false));
        let mut cache = ReadCache::new();
        let expect = as_approved(&fake);
        // One report across both reads, so the second cannot pass by inheriting an empty one: if
        // either path set a detail, it is still here to be seen.
        let mut report = ReadReport::new();
        let timed_out = handle_read(
            &fake,
            &mut cache,
            &refused,
        ...
        assert_eq!(timed_out, Some(ReadAnswer::Fail(FailReason::Timeout)));
        assert_eq!(report.detail, None);

        cancel.store(true, Ordering::SeqCst);
        let cancelled =
            handle_read(&fake, &mut cache, &refused, Budget::unlimited(), &cancel, Some(&expect), &mut report);
        assert_eq!(cancelled, None);
        assert_eq!(report.detail, None);
    }

    /// What a `failed` read is allowed to have measured. A recogniser error is the one failure that
    /// happens AFTER a capture, so it is the one with numbers to report — and they are numbers: the
    /// whole `ReadStats` struct is numbers and one boolean, and `protocol::read_line` narrows even
    /// that to the three that describe the capture.
    #[test]
    fn a_failure_after_the_capture_still_reports_what_the_capture_measured() {
        let mut fake = FakePlatform::new();
        fake.recognise_ok = false;
        let (answer, detail, stats) = read_detail(&fake);
        assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::Failed)));
        assert_eq!(detail, Some(FailDetail::RecogniseError));
        assert_eq!(stats.width, Some(FRAME_WIDTH as u64));
        assert_eq!(stats.height, Some(FRAME_HEIGHT as u64));
        assert!(stats.capture_ms.is_some());
        // Recognition never finished, so there is nothing honest to say about how long it took.
        assert_eq!(stats.recognise_ms, None);
    }

    /// And a failure BEFORE the capture has measured nothing, so it reports nothing — never a zero,
    /// which would read as "a capture that took no time".
    #[test]
    fn a_failure_before_the_capture_measured_nothing() {
        let fake = FakePlatform::new().shots([Shot::Err(CaptureError::Timeout)]);
        let (answer, detail, stats) = read_detail(&fake);
        assert_eq!(answer, Some(ReadAnswer::Fail(FailReason::Failed)));
        assert_eq!(detail, Some(FailDetail::CaptureTimeout));
        assert_eq!((stats.capture_ms, stats.width, stats.height), (None, None, None));
    }
    ...
    fn a_read_reports_the_frame_it_captured_and_whether_it_had_to_recognise() {
        ...
        handle_read(&fake, &mut cache, &refused, Budget::unlimited(), &cancel, Some(&approved), &mut first);
        assert_eq!((first.stats.width, first.stats.height), (Some(FRAME_WIDTH as u64), Some(FRAME_HEIGHT as u64)));
        assert_eq!(first.stats.cache_hit, Some(false));
        assert!(first.stats.capture_ms.is_some() && first.stats.recognise_ms.is_some());

        // The same pixels again: nothing is recognised, so there is no recognition time to report.
        let mut second = ReadReport::new();
        handle_read(&fake, &mut cache, &refused, Budget::unlimited(), &cancel, Some(&approved), &mut second);
        assert_eq!(second.stats.cache_hit, Some(true));
        assert_eq!(second.stats.recognise_ms, None);
    }
    ...
    fn a_cache_hit_has_no_fresh_geometry_to_report() {
        ...
        handle_read(&fake, &mut cache, &refused, budget, &cancel, Some(&expect), &mut ReadReport::measuring(&mut geometry));
        assert!(geometry.lines.is_some(), "the first read recognised, so it measured");

        // Read back through the report rather than through `geometry`, which it is still borrowing:
        // the point of the assertion is what the read WROTE, and the report is where it wrote it.
        let mut geometry = ReadGeometry::default();
        let mut report = ReadReport::measuring(&mut geometry);
        handle_read(&fake, &mut cache, &refused, budget, &cancel, Some(&expect), &mut report);
        assert_eq!(report.stats.cache_hit, Some(true));
        let measured = report.geometry.as_deref().expect("this read was given a geometry to fill in");
        assert_eq!(measured.lines, None, "a hit has no boxes of its own");
        assert_eq!(measured.band_px, None);
    }
